...
class Subjectivity:
    ...
    def _craft_response(self, context: str) -> str:
        ...
        if not bigrams:
            # fallback: split context into words and return first sentence
            sent = self._format_sentence(" ".join(words[:6]))
            return sent

        # Deterministic selection seeded by context
        seed = int(hashlib.sha256(context.encode("utf-8")).hexdigest(), 16)
        # Select up to 3 clauses deterministically
        clauses = []
        idx = 0
    ...
    def _join_clauses_with_punct(self, clauses: List[str], seed: int) -> str:
        # Make at least one non-period punctuation present for texture
        punct_schemes = [
            [", ", " — ", "."],
            ["; ", ", ", "."],
            [" — ", ", ", "."],
        ...
        out.append(".")  # ensure terminal period
        text = "".join(out)
        # Collapse accidental duplicate spaces
        text = re.sub(r"\s{2,}", " ", text)
        # Remove period duplication if scheme already ended with punctuation
