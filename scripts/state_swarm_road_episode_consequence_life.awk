# A.105: make each life one vote about episode endings and consequences.
...
BEGIN {
    FS = OFS = "\t"
    if (!min_episodes) min_episodes = 3
...
NR == 1 {
    if (NF != 29 || $1 != "candidate" || $4 != "cohort" ||
        $20 != "raw_ce_gain" || $26 != "unordered_ce_gain" ||
        $29 != "texture_snapshot_ce_gain") fail()
    next
}

{
    if (NF != 29 || $1 !~ /^consequence-[a-z0-9-]+$/ ||
    ...
    unordered_ce_gain[key] += $26; unordered_brier_gain[key] += $27
...
END {
    ...
    print "candidate", "strength", "rank", "cohort", "life", "split", \
        "episodes", "raw_episode_wins", "snapshot_episode_wins", \
        "texture_episode_wins", "unordered_episode_wins", \
        ...
        "social_consequence_ce_gain", "home_episodes", "storm_episodes", \
    ...
    for (p = 1; p <= policy_count; p++) {
        ...
        for (l = 1; l <= life_count; l++) {
            ...
            if (count[key] < min_episodes || count[key] > 4) fail()
            for (t = 1; t <= 4; t++) {
