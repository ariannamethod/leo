# A.100: make each covariance life one statistical vote.
...

{
    ...
    if ($18 > 0) prior_wins[key]++
    raw_ce_gain[key] += $16; raw_brier_gain[key] += $17
    prior_ce_gain[key] += $18; prior_brier_gain[key] += $19
...
END {
    ...
    print "candidate", "decay", "strength", "rank", "cohort", "life", \
        "split", "turns", "raw_turn_wins", "prior_turn_wins", \
        "mean_raw_ce_gain", "mean_raw_brier_gain", "mean_prior_ce_gain", \
        "mean_prior_brier_gain", "home_raw_ce_gain", "storm_raw_ce_gain", \
        "wonder_raw_ce_gain", "social_raw_ce_gain"
    for (p = 1; p <= policy_count; p++) {
        ...
        for (l = 1; l <= life_count; l++) {
            ...
            print candidate, decay[candidate], strength[candidate], rank[candidate], \
                ...
                prior_wins[key] + 0, sprintf("%.9f", raw_ce_gain[key] / count[key]), \
                sprintf("%.9f", raw_brier_gain[key] / count[key]), \
                sprintf("%.9f", prior_ce_gain[key] / count[key]), \
                sprintf("%.9f", prior_brier_gain[key] / count[key]), \
