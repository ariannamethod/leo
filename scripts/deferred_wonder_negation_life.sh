#!/usr/bin/env bash
# A.74: rejected meaning remains perceptible but cannot become a lesson.
set -euo pipefail

ROOT="$(cd "$(dirname "$0")/.." && pwd)"
CASES="$ROOT/scripts/deferred_wonder_negation_cases.tsv"
STAMP="$(date +%Y%m%d-%H%M%S)"
OUT="${1:-${TMPDIR:-/tmp}/leo-deferred-wonder-negation-$STAMP}"

if [ "${LEO_WONDER_NEGATION_PLAN_ONLY:-0}" = 1 ]; then
    cases="$(($(wc -l < "$CASES") - 1))"
    continued="$(
    ...
    [ "$cases" -eq 7 ] &&
        ...
        [ "$resolved" -eq 3 ] &&
        [ "$reasks" -eq 4 ] || {
        printf 'negation life cases do not satisfy the sealed plan\n' >&2
        exit 1
    }
    printf 'source\tcases\tcontinued\tresolved\treasks\tprocesses\tcontract\n'
    printf 'A.74\t%d\t%d\t%d\t%d\t%d\tassertion-is-not-perception\n' \
...
LEO_WONDER_NEGATION_PLAN_ONLY=1 "$0" > "$OUT/sealed-plan.tsv"
...
printf 'case\toutcome\tpending\tresolved\treask\tperceived_rejected\n' \
    > "$RESULTS"
tail -n +2 "$OUT/cases.tsv" |
while IFS=$'\t' read -r case_id answer expected_outcome \
        expected_pending expected_resolved expected_reask; do
    ...
    perceived=0
    case "$case_id" in
        negative-one|negative-both|contradiction)
            grep -Eq '\[flow: .* in=water\(' \
                "$case_dir/answer.log"
            perceived=1
            ;;
    esac
...
(
    ...
    printf '%s\n' \
        ...
        'c7fdd65559e993b5b65f014bc01fbf0a5d8f7f565e131f135e8666c08d2b11fe  results.tsv' \
