...
TMP="$(mktemp -d "${TMPDIR:-/tmp}/leo-road-delayed-outcome-receipt-decision.XXXXXX")"
...
write_lives() {
    ...
    for candidate in outcome-receipt-path-good outcome-receipt-path-carried outcome-receipt-path-bad; do
        case "$candidate" in
            outcome-receipt-path-good) strength=1; rank=1 ;;
            outcome-receipt-path-carried) strength=0.5; rank=2 ;;
...
run_decision() {
    ...
    awk -v policy_expected=3 -v discovery_expected=32 -v validation_expected=32 \
        -f "$ROOT/scripts/state_swarm_road_delayed_outcome_receipt_summary.awk" \
...
grep -q '^result independent-delayed-outcome-receipt-supported$' "$TMP/supported-verdict.txt"

run_decision validation-fail
grep -q $'^outcome-receipt-path-good\t1\t1\t32\t128\t32\t32\t32\t32\t32' \
    "$TMP/validation-fail-selection.tsv"
grep -q '^result independent-delayed-outcome-receipt-not-confirmed$' \
    "$TMP/validation-fail-verdict.txt"

run_decision none
grep -q $'^none\t0\t0\t32' "$TMP/none-selection.tsv"
grep -q '^result no-independent-delayed-outcome-receipt-candidate$' "$TMP/none-verdict.txt"

printf 'state-swarm road delayed outcome receipt decision: ok\n'
