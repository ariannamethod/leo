...
awk -F '\t' '
    NR > 1 { rows++; if ($8 == 40 || $8 == 41) exit 1; if ($22 > 0) order_gains++ }
    END { if (rows != 16 || order_gains == 0) exit 1 }
' "$TMP/scores.tsv"
...
if awk -v policy_expected=1 -v life_expected=1 -v writer_expected=24 \
    ...
    printf 'road ordered-episode reporter accepted forged runtime probability\n' >&2
    exit 1
fi

write_lives() {
    local mode="$1" candidate cohort life split i raw snapshot unordered
    printf 'candidate\tdecay\tstrength\trank\tcohort\tlife\tsplit\tturns\traw_turn_wins\tsnapshot_turn_wins\tunordered_turn_wins\tmean_raw_ce_gain\tmean_raw_brier_gain\tmean_snapshot_ce_gain\tmean_snapshot_brier_gain\tmean_unordered_ce_gain\tmean_unordered_brier_gain\tmean_snapshot_raw_ce_gain\thome_order_ce_gain\tstorm_order_ce_gain\twonder_order_ce_gain\tsocial_order_ce_gain\n'
    for candidate in path-good path-bad; do
        for cohort in discovery validation; do
            ...
            for ((i = 1; i <= limit; i++)); do
                ...
                raw=0.000000000; snapshot=0.000000000; unordered=0.000000000
                if [ "$mode" = supported ] && [ "$candidate" = path-good ]; then
                    raw=0.010000000; snapshot=0.003000000; unordered=0.001500000
                fi
                printf '%s\t0.5\t%s\t%s\t%s\t%s\t%s\t48\t48\t48\t48\t%s\t0.002000000\t%s\t0.001000000\t%s\t0.000400000\t0.007000000\t%s\t%s\t%s\t%s\n' \
                    "$candidate" "$([ "$candidate" = path-good ] && printf 1 || printf 0.25)" \
                    "$([ "$candidate" = path-good ] && printf 1 || printf 2)" "$cohort" \
                    "$life" "$split" "$raw" "$snapshot" "$unordered" \
...
run_decision() {
    ...
    awk -v policy_expected=2 -f "$ROOT/scripts/state_swarm_road_ordered_episode_select.awk" \
...
grep -q $'^path-good\t0.5\t1\t1\t12\t12\t12\t12' "$TMP/supported-selection.tsv"
grep -q '^result ordered-path-supported$' "$TMP/supported-verdict.txt"
run_decision none
grep -q $'^none\t0\t0' "$TMP/none-selection.tsv"
grep -q '^result no-ordered-path-candidate$' "$TMP/none-verdict.txt"

printf 'state-swarm road ordered-episode reporters: ok\n'
