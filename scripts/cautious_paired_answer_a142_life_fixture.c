...
static int inspect_word(
        const char *arm, const char *state, const char *word) {
    ...
    printf("%s\t%s\t%d\t%s\t%s\t%d\t%s\t%s\t%s\t%s\t%d\t%d\t%s\t%s\t%s\t%d\t%d\n",
           arm, word, leo_heard_count(&leo->heard, word),
...
int main(int argc, char **argv) {
    if (argc != 3) return 2;
    puts("arm\tword\theard\tlearned_primary\tlearned_alternate\tepisode\toffered_primary\toffered_alternate\tanswer_primary\tanswer_alternate\tresolved\treturns\tpending\tpending_primary\tpending_alternate\tpending_turns\twonders");
    if (inspect_word("control", argv[1], "simply")) return 3;
    if (inspect_word("control", argv[1], "difficult")) return 3;
    if (inspect_word("candidate", argv[2], "simply")) return 3;
    if (inspect_word("candidate", argv[2], "difficult")) return 3;
    return 0;
}
