# A.100: aggregate whichever covariance cohorts have actually been admitted.
...
NR == 1 {
    if (NF != 18 || $1 != "candidate" || $5 != "cohort" ||
        $11 != "mean_raw_ce_gain" || $18 != "social_raw_ce_gain") fail()
    next
}

{
    if (NF != 18 || $5 !~ /^(discovery|validation)$/ ||
        $7 !~ /^(primary|holdout)$/ || seen[$1 SUBSEP $5 SUBSEP $6]++) fail()
    for (i = 8; i <= 18; i++) if (!number($i)) fail()
    ...
    if ($13 > 0) prior_life_wins[key]++
    raw_ce_gain[key] += $11; raw_brier_gain[key] += $12
    prior_ce_gain[key] += $13; prior_brier_gain[key] += $14
    home_gain[key] += $15; storm_gain[key] += $16
    wonder_gain[key] += $17; social_gain[key] += $18
    split_lives[key SUBSEP $7]++
    if ($11 > 0) split_raw_wins[key SUBSEP $7]++
    if ($13 > 0) split_prior_wins[key SUBSEP $7]++
    split_raw_gain[key SUBSEP $7] += $11
    split_prior_gain[key SUBSEP $7] += $13
}
...
END {
    ...
    print "candidate", "decay", "strength", "rank", "cohort", "lives", \
        "turns", "raw_life_wins", "prior_life_wins", "mean_raw_ce_gain", \
        "mean_raw_brier_gain", "mean_prior_ce_gain", \
        "mean_prior_brier_gain", "home_raw_ce_gain", "storm_raw_ce_gain", \
        "wonder_raw_ce_gain", "social_raw_ce_gain", "primary_lives", \
        "holdout_lives", "primary_raw_wins", "holdout_raw_wins", \
        "primary_prior_wins", "holdout_prior_wins", "primary_mean_raw_gain", \
        "holdout_mean_raw_gain", "primary_mean_prior_gain", \
        "holdout_mean_prior_gain"
    for (p = 1; p <= policy_count; p++) {
        ...
        for (c = 1; c <= 2; c++) {
            ...
            print candidate, decay[candidate], strength[candidate], rank[candidate], \
                ...
                prior_life_wins[key] + 0, \
                sprintf("%.9f", raw_ce_gain[key] / lives[key]), \
                sprintf("%.9f", raw_brier_gain[key] / lives[key]), \
                sprintf("%.9f", prior_ce_gain[key] / lives[key]), \
                sprintf("%.9f", prior_brier_gain[key] / lives[key]), \
                ...
                split_prior_wins[key SUBSEP "primary"] + 0, \
                split_prior_wins[key SUBSEP "holdout"] + 0, \
                ...
                sprintf("%.9f", split_prior_gain[key SUBSEP "primary"] / split_lives[key SUBSEP "primary"]), \
                sprintf("%.9f", split_lives[key SUBSEP "holdout"] ? split_prior_gain[key SUBSEP "holdout"] / split_lives[key SUBSEP "holdout"] : 0)
        }
    }
}
