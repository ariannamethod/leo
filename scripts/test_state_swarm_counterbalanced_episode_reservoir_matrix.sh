...
TMP="$(mktemp -d "${TMPDIR:-/tmp}/leo-counterbalanced-reservoir-plan.XXXXXX")"
...
awk -F '\t' '
    ...
    }
    {
        ...
        expected_seed = 275283 + (rows - 1) * 1033
        if ($1 != expected_life || $2 != expected_split || $3 != expected_seed || seed[$3]++) exit 1
    }
    END { if (rows != 80) exit 1 }
' "$CANDIDATES"

previous_last="$(tail -n 1 "$ROOT/scripts/state_swarm_renewal_event_reservoir_candidates.tsv" | awk -F '\t' '{ print $3 }')"
...
LEO_STATE_PROSPECTIVE_PLAN_ONLY=1 \
    "$ROOT/scripts/state_swarm_counterbalanced_episode_reservoir_matrix.sh" "$TMP/run" \
    > "$TMP/plan.stdout"
...
printf 'state-swarm counterbalanced episode reservoir plan: ok\n'
