# A.102: make each sequence-error life one statistical vote.
...
NR == 1 {
    if (NF != 21 || $1 != "candidate" || $5 != "cohort" ||
        $16 != "raw_ce_gain" || $21 != "snapshot_raw_brier_gain") fail()
    next
}

{
    if (NF != 21 || $1 !~ /^seq-[a-z0-9-]+$/ ||
    ...
    if ($18 > 0) snapshot_wins[key]++
    raw_ce_gain[key] += $16; raw_brier_gain[key] += $17
    snapshot_ce_gain[key] += $18; snapshot_brier_gain[key] += $19
    snapshot_raw_ce_gain[key] += $20; snapshot_raw_brier_gain[key] += $21
...
END {
    ...
    print "candidate", "decay", "strength", "rank", "cohort", "life", \
        ...
        "mean_snapshot_brier_gain", "mean_snapshot_raw_ce_gain", \
    ...
    for (p = 1; p <= policy_count; p++) {
        ...
        for (l = 1; l <= life_count; l++) {
            ...
            print candidate, decay[candidate], strength[candidate], rank[candidate], \
                ...
                raw_wins[key] + 0, snapshot_wins[key] + 0, \
