#!/usr/bin/env bash
# A.109 source: untouched Williams-balanced lives immediately after A.108-r2.
set -Eeuo pipefail

ROOT="$(cd "$(dirname "$0")/.." && pwd)"
STAMP="$(date +%Y%m%d-%H%M%S)"
OUT="${1:-${TMPDIR:-/tmp}/leo-state-swarm-outcome-receipt-reservoir-$STAMP}"

export LEO_STATE_PROSPECTIVE_CANDIDATES="$ROOT/scripts/state_swarm_outcome_receipt_reservoir_candidates.tsv"
...
export LEO_STATE_PROSPECTIVE_SEED_START=605843
...
export LEO_STATE_PROSPECTIVE_PROFILE_NAME=A.109-delayed-outcome-receipt-r1
