#!/usr/bin/env bash
# A.102: test whether state velocity predicts error left by the frozen A.101 snapshot.
set -Eeuo pipefail

trap 'rc=$?; printf "road sequence-error runner failed: line=%s rc=%s command=%s\n" "$LINENO" "$rc" "$BASH_COMMAND" >&2; exit "$rc"' ERR

ROOT="$(cd "$(dirname "$0")/.." && pwd)"
CC="${CC:-cc}"
A89="${LEO_STATE_SEQUENCE_ERROR_A89_SOURCE:-/private/tmp/leo-state-swarm-balanced-event-reservoir-a89-r1-20260807}"
A101="${LEO_STATE_SEQUENCE_ERROR_A101_SOURCE:-/private/tmp/leo-state-swarm-road-error-memory-a101-r1-20260810}"
JOBS="${LEO_STATE_SEQUENCE_ERROR_JOBS:-3}"
AGGREGATE_ONLY="${LEO_STATE_SEQUENCE_ERROR_AGGREGATE_ONLY:-0}"
STAMP="$(date +%Y%m%d-%H%M%S)"
OUT="${1:-${TMPDIR:-/tmp}/leo-state-swarm-road-sequence-error-$STAMP}"
...
A101_SOURCE_RECEIPT="$A101/source-receipt.tsv"
A101_DISCOVERY_SUMMARY="$A101/discovery-summary.tsv"
A101_SELECTION="$A101/selection.tsv"
A101_VERDICT="$A101/verdict.txt"

EXPECTED_SCREEN_SHA="${LEO_STATE_SEQUENCE_ERROR_SCREEN_SHA:-017ab2f9239ca20cc51f1691cf9175f4a5ca36e7e337a654654791f86a81ea8f}"
EXPECTED_WARM_SHA="${LEO_STATE_SEQUENCE_ERROR_WARM_SHA:-02a7a18552ed850b3f7844ee75ca9bfedff5f9f6b750db293fbe81e271c51840}"
EXPECTED_ENROLLMENT_SHA="${LEO_STATE_SEQUENCE_ERROR_ENROLLMENT_SHA:-d4f32d34a89819edb7428c06185889dfeb6648911571f4234f9ecdc5354df1fc}"
EXPECTED_WRITER_PLAN_SHA="${LEO_STATE_SEQUENCE_ERROR_WRITER_PLAN_SHA:-1cb1301c20e5124bc9e0493a354c6eb83347eea6c14665181c366ef629df43b3}"
EXPECTED_WRITER_SHA="${LEO_STATE_SEQUENCE_ERROR_WRITER_SHA:-10bf4251848f4edaec4a34d66d3d811afff187df9f10aeb71ff761a342c0e833}"
EXPECTED_A101_POLICIES_SHA="${LEO_STATE_SEQUENCE_ERROR_A101_POLICIES_SHA:-89736f8e4681db85b8c516f8095c16786d2e13ed1edbd4d3203244baff5529ab}"
EXPECTED_A101_DISCOVERY_PLAN_SHA="${LEO_STATE_SEQUENCE_ERROR_A101_DISCOVERY_PLAN_SHA:-ebe356a8b3055c6467a163a38e3893c115bfd39ca13ab6cff56ac802f013b860}"
EXPECTED_A101_VALIDATION_PLAN_SHA="${LEO_STATE_SEQUENCE_ERROR_A101_VALIDATION_PLAN_SHA:-259be3140c58c7706f985283734cc0c57b99d2d46f6221c17ee0adad6c026a82}"
EXPECTED_A101_SOURCE_RECEIPT_SHA="${LEO_STATE_SEQUENCE_ERROR_A101_SOURCE_RECEIPT_SHA:-291e2d7bd183a5937976ba1d9798a9176dabf6e83f84d53cb2fbe57638d4bb53}"
EXPECTED_A101_DISCOVERY_SUMMARY_SHA="${LEO_STATE_SEQUENCE_ERROR_A101_DISCOVERY_SUMMARY_SHA:-05daa176f90ece7e6eb94016a9bb8e1122f7df0f1c6b2528e8d68643bb8fbb7a}"
EXPECTED_A101_SELECTION_SHA="${LEO_STATE_SEQUENCE_ERROR_A101_SELECTION_SHA:-0933c24077634e0e993a4019c628eecb5331138ebd6a26bde4eed99741c3364b}"
EXPECTED_A101_VERDICT_SHA="${LEO_STATE_SEQUENCE_ERROR_A101_VERDICT_SHA:-24053340a58e648b569de83255970334ad4fbb87e2f3d694bb1187203c0ffb29}"

case "$JOBS" in
    ''|*[!0-9]*|0) printf 'invalid road sequence-error jobs: %s\n' "$JOBS" >&2; exit 2 ;;
esac
...
seal "$A101_VALIDATION_PLAN" "$EXPECTED_A101_VALIDATION_PLAN_SHA" "A.101 unopened validation plan"
seal "$A101_SOURCE_RECEIPT" "$EXPECTED_A101_SOURCE_RECEIPT_SHA" "A.101 source receipt"
...
write_discovery_plan() {
    awk -F '\t' -v OFS='\t' '
        BEGIN {
            split("h25 h26 h27 h28 h29 h30", life, " ")
            split("176115 177148 178181 179214 180247 181280", seed, " ")
            split("65 66 67 68 69 70", order, " ")
            split("24 25 26 27 28 29", rank, " ")
        }
        NR == 1 {
            if (NF != 5 || $1 != "life" || $5 != "enrollment_rank") exit 2
            print "cohort", $1, "split", "base_seed", "candidate_order", $5
            next
        }
        $1 ~ /^h(25|26|27|28|29|30)$/ {
            rows++
            if ($1 != life[rows] || $2 != "holdout" || $3 != seed[rows] ||
                $4 != order[rows] || $5 != rank[rows]) exit 2
            print "discovery", $1, $2, $3, $4, $5
        }
...
write_validation_plan() {
    ...
    ' "$A101_VALIDATION_PLAN"
}

write_policies() {
    printf 'candidate\tsequence_decay\tsequence_strength\tsnapshot_decay\tsnapshot_strength\tprior_alpha\tvariance_ridge\trank\n'
    printf 'seq-cumulative-light\t1.00\t0.10\t1.00\t0.25\t1\t1\t1\n'
    printf 'seq-cumulative-gentle\t1.00\t0.25\t1.00\t0.25\t1\t1\t2\n'
    printf 'seq-slow-light\t0.97\t0.10\t1.00\t0.25\t1\t1\t3\n'
    printf 'seq-slow-gentle\t0.97\t0.25\t1.00\t0.25\t1\t1\t4\n'
    printf 'seq-fast-light\t0.90\t0.10\t1.00\t0.25\t1\t1\t5\n'
    printf 'seq-fast-gentle\t0.90\t0.25\t1.00\t0.25\t1\t1\t6\n'
}
...
write_source_receipt() {
    ...
    printf 'a101-source-receipt\t%s\n' "$EXPECTED_A101_SOURCE_RECEIPT_SHA"
...
if [ "$AGGREGATE_ONLY" = 1 ]; then
    for path in "$DISCOVERY_PLAN" "$VALIDATION_PLAN" "$POLICIES" \
        ...
        [ -s "$path" ] || {
            printf 'incomplete road sequence-error run: %s\n' "$OUT" >&2
            exit 2
        }
    done
    write_discovery_plan | cmp -s - "$DISCOVERY_PLAN" || {
        printf 'A.102 discovery plan diverged\n' >&2; exit 2;
    }
    write_validation_plan | cmp -s - "$VALIDATION_PLAN" || {
        printf 'A.102 validation plan diverged\n' >&2; exit 2;
    }
    write_policies | cmp -s - "$POLICIES" || {
        printf 'A.102 policy ledger diverged\n' >&2; exit 2;
    }
    write_source_receipt | cmp -s - "$SOURCE_RECEIPT" || {
        printf 'A.102 source receipt diverged\n' >&2; exit 2;
    }
else
    [ ! -e "$OUT" ] || {
...
if [ "${LEO_STATE_SEQUENCE_ERROR_PLAN_ONLY:-0}" = 1 ]; then
    cat "$DISCOVERY_PLAN"
    printf '\n'
    cat "$VALIDATION_PLAN"
    printf '\n'
...
if [ "$AGGREGATE_ONLY" != 1 ]; then
    ...
    FIXTURE="$OUT/road-sequence-error-fixture"
...
[ -n "$selected" ] || {
    printf 'A.102 selection is empty\n' >&2
    exit 2
}
...
if [ "$selected" != none ]; then
    ...
    [ "$(wc -l < "$SELECTED_POLICY" | tr -d ' ')" -eq 2 ] || {
        printf 'selected sequence-error policy is not unique\n' >&2
        exit 2
    }
    ...
    awk -v policy_expected=1 -v life_expected=10 -v writer_expected=64 \
        ...
        > "$VALIDATION_SCORES"
    awk -v policy_expected=1 -v life_expected=10 -v min_turns=48 \
        -f "$ROOT/scripts/state_swarm_road_sequence_error_life.awk" \
