# A.109: ask whether an unexpected realized consequence survives the signed path that followed it.

function fail() {
    if (!fatal) print "road delayed-outcome-receipt report rejected " FILENAME ":" FNR > "/dev/stderr"
    fatal = 1
    exit 2
}
...
function start_epoch(life,    candidate, texture, i, j, k, key, current_epoch) {
    ...
    for (candidate in policy_seen) {
        ...
        for (k = 1; k <= 24; k++) {
            ...
            path_mean[key SUBSEP k] = 0
            path_variance[key SUBSEP k] = 0
            for (j = 1; j <= 8; j++)
                path_covariance[key SUBSEP k SUBSEP j] = 0
        }
        for (k = 1; k <= 4; k++) {
        ...
        for (j = 1; j <= 8; j++) {
            ...
            path_error_mean[key SUBSEP j] = 0
...
FILENAME == ARGV[1] {
    ...
    if (NF != 8 || $1 !~ /^outcome-receipt-path-[a-z0-9-]+$/ ||
        policy_seen[$1]++ ||
...
FNR == 1 {
    if (NF != 31 || $1 != "cohort" || $4 != "turn" ||
        ...
        $22 != "observed_grounded" || $25 != "observed_alignment_delta" ||
        $26 != "forecast_grounded" || $29 != "forecast_alignment_delta" ||
        $31 != "reply") fail()
    next
}

{
    ...
    if (NF != 31 || !(life in lock_seen) || $1 != life_cohort[life] ||
        ...
        $30 == "" || $31 == "") fail()
    if ($17 == 1) {
        if (!number($26) || $26 < 0 || $26 > 1 ||
            !number($27) || $27 < -1 || $27 > 1 ||
            !number($28) || $28 < -1 || $28 > 1 ||
            !number($29) || $29 < -1 || $29 > 1) fail()
    } else if ($26 != "na" || $27 != "na" || $28 != "na" || $29 != "na") fail()
    ...
    if ($6 == 1 && $5 > 1 && session_rows[previous_key] == 8 &&
        ...
        for (i = 1; i <= 8; i++) {
            ...
            path_previous[session_key SUBSEP i] = target[i]
        }
        for (i = 1; i <= 4; i++) {
            outcome_scale = i == 1 ? 1 : 2
            boundary_outcome_receipt[session_key SUBSEP i] = \
                ($(21 + i) - $(25 + i)) / outcome_scale
        }
    ...
    if (boundary_valid[session_key] && $6 >= 2 && $6 <= 4)
        for (i = 1; i <= 8; i++) {
            ...
            path_feature[($6 - 2) * 8 + i] = \
                target[i] - path_previous[session_key SUBSEP i]
            path_previous[session_key SUBSEP i] = target[i]
        }
    ...
    if (receipt_ready) {
        ...
        for (i = 1; i <= 4; i++)
            receipt_feature[i] = boundary_outcome_receipt[session_key SUBSEP i]
        ...
        for (p = 1; p <= policy_count; p++) {
            ...
            for (j = 1; j <= 8; j++) {
                path_prediction[j] = symmetric_prediction[j]
                for (k = 1; k <= 24; k++) {
                    feature_key = reader_key SUBSEP k
                    path_prediction[j] += strength[candidate] * \
                        (path_feature[k] - path_mean[feature_key]) * \
                        path_covariance[feature_key SUBSEP j] / \
                        (ridge + path_variance[feature_key])
                }
            }
            normalize(path_prediction)
            for (j = 1; j <= 8; j++) {
                receipt_prediction[j] = path_prediction[j]
                for (k = 1; k <= 4; k++) {
                    feature_key = reader_key SUBSEP k
            ...
            path_ce = ce(target, path_prediction)
            path_brier = brier(target, path_prediction)
            ...
            if ($5 >= evaluation_session) {
                if (!header++)
                    print "candidate", "receipt_strength", "rank", "cohort", \
                        ...
                        "receipt_ce", "path_ce", "symmetric_ce", "texture_ce", \
                        "snapshot_ce", "raw_brier", "receipt_brier", "path_brier", \
                        "symmetric_brier", "texture_brier", "snapshot_brier", "raw_ce_gain", \
                        "raw_brier_gain", "snapshot_ce_gain", "snapshot_brier_gain", \
                        "texture_ce_gain", "texture_brier_gain", "symmetric_ce_gain", \
                        "symmetric_brier_gain", "path_ce_gain", "path_brier_gain", \
                        "snapshot_raw_ce_gain", "texture_snapshot_ce_gain", \
                        "path_symmetric_ce_gain"
                print candidate, strength[candidate], rank[candidate], $1, life, $3, \
                    $4, $5, $7, sprintf("%.9f", raw_ce), \
                    sprintf("%.9f", receipt_ce), sprintf("%.9f", path_ce), \
                    sprintf("%.9f", symmetric_ce), \
                    ...
                    sprintf("%.9f", path_ce - receipt_ce), \
                    sprintf("%.9f", path_brier - receipt_brier), \
            ...
            for (j = 1; j <= 8; j++) {
                ...
                path_error_delta[j] = target[j] - symmetric_prediction[j] - \
                    path_error_mean[reader_key SUBSEP j]
                receipt_error_delta[j] = target[j] - path_prediction[j] - \
            ...
            for (k = 1; k <= 24; k++) {
                ...
                path_variance[feature_key] += recenter * path_delta[k] * path_delta[k]
                for (j = 1; j <= 8; j++)
                    path_covariance[feature_key SUBSEP j] += \
                        recenter * path_delta[k] * path_error_delta[j]
            }
            for (k = 1; k <= 4; k++) {
                feature_key = reader_key SUBSEP k
            ...
            for (k = 1; k <= 24; k++)
                path_mean[reader_key SUBSEP k] += path_delta[k] / new_weight
            for (k = 1; k <= 4; k++)
            ...
            for (j = 1; j <= 8; j++) {
                ...
                path_error_mean[reader_key SUBSEP j] += path_error_delta[j] / new_weight
