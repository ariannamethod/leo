...
TMP="$(mktemp -d "${TMPDIR:-/tmp}/leo-receipt-path-reservoir-plan.XXXXXX")"
...
awk -F '\t' '
    ...
    }
    {
        ...
        expected_seed = 523203 + (rows - 1) * 1033
        if ($1 != expected_life || $2 != expected_split || $3 != expected_seed || seed[$3]++) exit 1
    }
    END { if (rows != 80) exit 1 }
' "$CANDIDATES"

a107_last="$(tail -n 1 "$ROOT/scripts/state_swarm_delayed_receipt_reservoir_candidates.tsv" | awk -F '\t' '{ print $3 }')"
new_first="$(sed -n '2p' "$CANDIDATES" | awk -F '\t' '{ print $3 }')"
[ "$new_first" -eq $((a107_last + 81 * 1033)) ]
...
LEO_STATE_PROSPECTIVE_PLAN_ONLY=1 \
    "$ROOT/scripts/state_swarm_receipt_path_reservoir_matrix.sh" "$TMP/run" \
    > "$TMP/plan.stdout"
...
printf 'state-swarm receipt path reservoir plan: ok\n'
