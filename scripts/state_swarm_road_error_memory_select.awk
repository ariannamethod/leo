# A.101: nominate conditional error memory on sealed discovery lives only.
...
BEGIN {
    ...
    if (!bias_win_required) bias_win_required = 4
...
NR == 1 {
    if (NF != 28 || $1 != "candidate" || $5 != "cohort" ||
        $8 != "raw_life_wins" || $28 != "holdout_mean_bias_gain") fail()
    next
}

$5 == "discovery" {
    if (NF != 28 || seen[$1]++ || $6 != expected || $19 != expected ||
        $20 != 0 || !number($10) || !number($11) || !number($12) ||
        !number($13)) fail()
    rows++
    if ($8 >= raw_win_required && $9 >= bias_win_required &&
        $10 >= ce_required && $11 >= brier_required &&
        $12 >= bias_ce_required && $13 >= bias_brier_required &&
        $15 > 0 && $16 > 0 && $17 > 0 && $18 > 0) {
        qualified++
        if (!selected || $10 > best_gain + 0.0000000005 ||
            ...
            best_raw_wins = $8; best_bias_wins = $9
            best_gain = $10; best_brier = $11
            best_bias_gain = $12; best_bias_brier = $13
            best_bias_raw = $14; best_home = $15; best_storm = $16
...
END {
    ...
    print "candidate", "decay", "strength", "rank", "discovery_lives", \
        "raw_life_wins", "bias_life_wins", "mean_raw_ce_gain", \
    ...
    if (selected)
        print selected, best_decay, best_strength, best_rank, expected, \
            best_raw_wins, best_bias_wins, best_gain, best_brier, \
            best_bias_gain, best_bias_brier, best_bias_raw, best_home, \
            best_storm, best_wonder, best_social, qualified, "candidate-nominated"
    else
        print "none", 0, 0, 0, expected, 0, 0, "0.000000000", \
            "0.000000000", "0.000000000", "0.000000000", "0.000000000", \
            "0.000000000", "0.000000000", "0.000000000", "0.000000000", \
            0, "no-error-memory-candidate"
}
