# A.109: confirm one frozen unexpected-outcome receipt beyond a full signed-path control.
...
END {
    ...
    if (selection_result == "no-independent-delayed-outcome-receipt-candidate") {
        if (selected != "none" || qualified != 0) fail()
        result = "no-independent-delayed-outcome-receipt-candidate"
    } else {
        ...
        if (raw_wins >= life_win_required && snapshot_wins >= life_win_required &&
            ...
            if (primary_path > 0 && holdout_path > 0)
                result = "independent-delayed-outcome-receipt-supported"
            else
                result = "independent-delayed-outcome-receipt-not-confirmed"
        else
            result = "independent-delayed-outcome-receipt-not-confirmed"
    }
