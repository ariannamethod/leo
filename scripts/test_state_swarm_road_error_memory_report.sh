...
TMP="$(mktemp -d "${TMPDIR:-/tmp}/leo-road-error-memory-report.XXXXXX")"
trap 'rm -rf "$TMP"' EXIT

printf 'candidate\tdecay\tstrength\tprior_alpha\tvariance_ridge\trank\n' > "$TMP/policies.tsv"
printf 'err-good\t1.00\t1\t1\t1\t1\n' >> "$TMP/policies.tsv"
...
for ((turn = 33; turn <= 44; turn++)); do
    ...
    event=updated
    replaced=0
    member_id="$([ "$turn" -lt 36 ] && printf '8' || printf '9')"
    if [ $((turn % 2)) -eq 1 ]; then
        source='1/0/0/0/0/0/0/0'
        members="1:0.000,2:1.000,3:0.000,4:0.000,5:0.000,6:0.000,7:0.000,$member_id:0.000"
        winner=2
    else
        source='0/1/0/0/0/0/0/0'
        members="1:1.000,2:0.000,3:0.000,4:0.000,5:0.000,6:0.000,7:0.000,$member_id:0.000"
        winner=1
    fi
    if [ "$turn" -eq 36 ]; then
        event=replaced
...
if awk -v policy_expected=1 -v life_expected=1 -v writer_expected=12 \
    ...
    >/dev/null 2>&1; then
    printf 'road error-memory reporter accepted forged runtime probability\n' >&2
    exit 1
fi

write_lives() {
    local mode="$1" candidate cohort life split i raw bias
    printf 'candidate\tdecay\tstrength\trank\tcohort\tlife\tsplit\tturns\traw_turn_wins\tbias_turn_wins\tmean_raw_ce_gain\tmean_raw_brier_gain\tmean_bias_ce_gain\tmean_bias_brier_gain\tmean_bias_raw_ce_gain\thome_raw_ce_gain\tstorm_raw_ce_gain\twonder_raw_ce_gain\tsocial_raw_ce_gain\n'
    for candidate in err-good err-bad; do
        for cohort in discovery validation; do
            ...
            for ((i = 1; i <= limit; i++)); do
                ...
                raw=0.000000000; bias=0.000000000
                if [ "$mode" = supported ] && [ "$candidate" = err-good ]; then
                    raw=0.010000000; bias=0.003000000
                fi
                printf '%s\t1\t%s\t%s\t%s\t%s\t%s\t6\t6\t6\t%s\t0.002000000\t%s\t0.001000000\t0.007000000\t%s\t%s\t%s\t%s\n' \
                    "$candidate" "$([ "$candidate" = err-good ] && printf 1 || printf 0.25)" \
                    "$([ "$candidate" = err-good ] && printf 1 || printf 2)" \
                    "$cohort" "$life" "$split" "$raw" "$bias" \
                    "$raw" "$raw" "$raw" "$raw"
            done
        done
    done
...
grep -q $'^err-good\t1\t1\t1\t6\t6\t6' "$TMP/supported-selection.tsv"
grep -q '^result conditional-error-memory-supported$' "$TMP/supported-verdict.txt"

run_decision none
grep -q $'^none\t0\t0' "$TMP/none-selection.tsv"
grep -q '^result no-error-memory-candidate$' "$TMP/none-verdict.txt"

printf 'state-swarm road error-memory reporters: ok\n'
