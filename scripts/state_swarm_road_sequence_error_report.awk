# A.102: ask whether recent state motion predicts error left by A.101's snapshot.

function fail() {
    if (!fatal) print "road sequence-error report rejected " FILENAME ":" FNR > "/dev/stderr"
    fatal = 1
    exit 2
}
...
function start_epoch(life,    candidate, i, j, key, current_epoch) {
    ...
    for (candidate in policy_seen)
        sequence_weight[candidate SUBSEP life SUBSEP current_epoch] = alpha[candidate]
    has_previous[life] = 0
}
...
BEGIN {
    ...
    if (!score_min) score_min = 48
}

FILENAME == ARGV[1] {
    if (FNR == 1) {
        if (NF != 8 || $1 != "candidate" || $2 != "sequence_decay" ||
            $4 != "snapshot_decay" || $6 != "prior_alpha" || $8 != "rank") fail()
        next
    }
    if (NF != 8 || $1 !~ /^seq-[a-z0-9-]+$/ || policy_seen[$1]++ ||
    ...
    sequence_decay[$1] = $2 + 0
    sequence_strength[$1] = $3 + 0
    alpha[$1] = $6 + 0
    ridge[$1] = $7 + 0
    rank[$1] = $8 + 0
    if (policy_count == 1) {
...

{
    ...
    sequence_ready = has_previous[life]
    if (sequence_ready)
        for (i = 1; i <= 8; i++) velocity[i] = source[i] - previous_source[life SUBSEP i]
    ...
    for (p = 1; p <= policy_count; p++) {
        ...
        for (j = 1; j <= 8; j++) {
            ...
            if (sequence_ready)
                for (i = 1; i <= 8; i++) {
                    sequence_key = candidate SUBSEP life SUBSEP current_epoch SUBSEP pre_id[i]
                    centered_velocity = velocity[i] - velocity_mean[sequence_key]
                    candidate_prediction[j] += sequence_strength[candidate] * \
                        centered_velocity * sequence_covariance[sequence_key SUBSEP pre_id[j]] / \
                        (ridge[candidate] + velocity_variance[sequence_key])
                }
        }
        ...
        if (sequence_ready && $4 >= evaluation_start) {
            if (!header++)
                print "candidate", "sequence_decay", "sequence_strength", "rank", \
            ...
            print candidate, sequence_decay[candidate], sequence_strength[candidate], \
        ...
        if (sequence_ready) {
            ...
            old_weight = sequence_weight[weight_key]
            new_weight = sequence_decay[candidate] * old_weight + 1
            recenter = sequence_decay[candidate] * old_weight / new_weight
            for (i = 1; i <= 8; i++) {
                sequence_key = candidate SUBSEP life SUBSEP current_epoch SUBSEP pre_id[i]
                velocity_delta[i] = velocity[i] - velocity_mean[sequence_key]
            }
            for (j = 1; j <= 8; j++) {
                destination_key = candidate SUBSEP life SUBSEP current_epoch SUBSEP pre_id[j]
                sequence_error[j] = target[j] - snapshot_prediction[j]
                sequence_error_delta[j] = sequence_error[j] - sequence_error_mean[destination_key]
            }
            for (i = 1; i <= 8; i++) {
                sequence_key = candidate SUBSEP life SUBSEP current_epoch SUBSEP pre_id[i]
                velocity_variance[sequence_key] = sequence_decay[candidate] * \
                    velocity_variance[sequence_key] + recenter * velocity_delta[i] * velocity_delta[i]
                for (j = 1; j <= 8; j++)
                    sequence_covariance[sequence_key SUBSEP pre_id[j]] = \
                        sequence_decay[candidate] * sequence_covariance[sequence_key SUBSEP pre_id[j]] + \
                        recenter * velocity_delta[i] * sequence_error_delta[j]
            }
            for (i = 1; i <= 8; i++) {
                sequence_key = candidate SUBSEP life SUBSEP current_epoch SUBSEP pre_id[i]
                velocity_mean[sequence_key] += velocity_delta[i] / new_weight
            }
            for (j = 1; j <= 8; j++) {
                destination_key = candidate SUBSEP life SUBSEP current_epoch SUBSEP pre_id[j]
                sequence_error_mean[destination_key] += sequence_error_delta[j] / new_weight
            }
            sequence_weight[weight_key] = new_weight
        }
    }
    ...
    for (i = 1; i <= 8; i++) {
        ...
        previous_source[life SUBSEP i] = source[i]
    }
    for (j = 1; j <= 8; j++) {
        destination_key = life SUBSEP current_epoch SUBSEP pre_id[j]
