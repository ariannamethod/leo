# A.107: aggregate delayed-receipt lives without pooling their votes.
...
NR == 1 {
    if (NF != 29 || $1 != "candidate" || $4 != "cohort" ||
        $12 != "mean_raw_ce_gain" || $29 != "social_receipts") fail()
    next
}

{
    ...
    if ($18 > 0) symmetric_life_wins[key]++
    ...
    symmetric_ce_gain[key] += $18; symmetric_brier_gain[key] += $19
