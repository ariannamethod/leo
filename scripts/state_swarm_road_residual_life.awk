# A.99: keep turns correlated inside one life and let each life vote once.
...
NR == 1 {
    if (NF != 23 || $1 != "candidate" || $5 != "cohort" ||
        $18 != "raw_ce_gain" || $23 != "destination_brier_gain") fail()
    next
}

{
    if (NF != 23 || $1 !~ /^excess-[a-z0-9-]+$/ ||
        !number($2) || !number($3) || !number($4) ||
        $5 !~ /^(discovery|validation)$/ || $6 !~ /^[ph][0-9][0-9]$/ ||
        $7 !~ /^(primary|holdout)$/ || $8 !~ /^[0-9]+$/ ||
        $9 !~ /^(home|storm|wonder|social)$/) fail()
    for (i = 10; i <= 23; i++) if (!number($i)) fail()
    ...
    if ($20 > 0) control_wins[key]++
    raw_ce_gain[key] += $18; raw_brier_gain[key] += $19
    control_ce_gain[key] += $20; control_brier_gain[key] += $21
    destination_ce_gain[key] += $22; destination_brier_gain[key] += $23
...
END {
    ...
    print "candidate", "decay", "strength", "rank", "cohort", "life", \
        "split", "turns", "raw_turn_wins", "control_turn_wins", \
        "mean_raw_ce_gain", "mean_raw_brier_gain", "mean_control_ce_gain", \
    ...
    for (p = 1; p <= policy_count; p++) {
        ...
        for (l = 1; l <= life_count; l++) {
            ...
            print candidate, decay[candidate], strength[candidate], rank[candidate], \
                cohort[life], life, split_name[life], count[key], raw_wins[key] + 0, \
                control_wins[key] + 0, sprintf("%.9f", raw_ce_gain[key] / count[key]), \
                sprintf("%.9f", raw_brier_gain[key] / count[key]), \
                sprintf("%.9f", control_ce_gain[key] / count[key]), \
                sprintf("%.9f", control_brier_gain[key] / count[key]), \
                sprintf("%.9f", destination_ce_gain[key] / count[key]), \
                sprintf("%.9f", destination_brier_gain[key] / count[key]), \
