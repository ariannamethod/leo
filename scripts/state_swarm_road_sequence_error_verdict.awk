# A.102: confirm only a frozen sequence correction on ten untouched lives.
...
FILENAME == ARGV[1] {
    ...
    discovery_snapshot_gain = $10; discovery_snapshot_brier = $11
...
FNR == 1 {
    if (NF != 28 || $1 != "candidate" || $5 != "cohort" ||
        $28 != "holdout_mean_snapshot_gain") fail()
    next
}
...
$5 == "validation" && $1 == selected {
    ...
    validation_snapshot_gain = $12; validation_snapshot_brier = $13
    home_gain = $15; storm_gain = $16; wonder_gain = $17; social_gain = $18
    primary_raw_wins = $21; holdout_raw_wins = $22
    primary_snapshot_wins = $23; holdout_snapshot_wins = $24
    primary_raw = $25; holdout_raw = $26
    primary_snapshot = $27; holdout_snapshot = $28
}

END {
    if (fatal) exit 2
    if (selection_rows != 1) fail()
    if (selection_result == "no-sequence-error-candidate") {
        if (selected != "none" || qualified != 0) fail()
        result = "no-sequence-error-candidate"
    } else {
        ...
        if (validation_raw_wins >= raw_win_required &&
            ...
            primary_snapshot_wins >= split_win_required &&
            holdout_snapshot_wins >= split_win_required &&
            ...
            result = "sequence-error-memory-supported"
        else
            result = "sequence-error-memory-not-confirmed"
    }
