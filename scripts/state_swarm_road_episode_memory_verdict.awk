# A.103: confirm only a frozen episode reader on ten untouched lives.
...
END {
    ...
    if (selection_result == "no-episode-memory-candidate") {
        if (selected != "none" || qualified != 0) fail()
        result = "no-episode-memory-candidate"
    } else {
        ...
        if (validation_raw_wins >= raw_win_required &&
            ...
            result = "episode-memory-supported"
        else
            result = "episode-memory-not-confirmed"
    }
