#!/usr/bin/env bash
# A.134: let a visible-only interlocutor continue the real A.133 turn-14 fork.
set -Eeuo pipefail

trap 'rc=$?; printf "responsive A.133 continuation failed: line=%s rc=%s command=%s\n" "$LINENO" "$rc" "$BASH_COMMAND" >&2; exit "$rc"' ERR

ROOT="$(cd "$(dirname "$0")/.." && pwd)"
STAMP="$(date +%Y%m%d-%H%M%S)"
OUT="${1:-${TMPDIR:-/tmp}/leo-responsive-a133-continuation-a134-$STAMP}"
KEY_FILE="${OPENAI_API_KEY_FILE:-}"
MODEL="${LEO_INTERLOCUTOR_MODEL:-gpt-5.6-luna}"
PREFIX="$ROOT/scripts/fixtures/responsive_a133_continuation_a134_prefix.txt"
...
if [ ! -f "$API/dialogue.jsonl" ]; then
    LEO_NATURAL_REPLAY_FILE="$PREFIX" \
        ...
        LEO_NATURAL_QUESTION=responsive-a133-continuation \
...
jq -e '.source == "frozen-prefix-then-responses-api-visible-transcript" and
       .replay_prefix_turns == 14 and .api_turns == 10 and
...
resolved_model="$(awk -F '\t' '$3 == 15 { print $8 }' "$API/sessions.tsv")"
printf 'meal\t617\t24\t14\t10\t%s\ttrue\ttrue\t%s\t%s\t%s\t%s\t%s\t%s\n' \
    "$resolved_model" \
...
cat "$MATRIX"
printf 'result\tresponsive-a133-continuation-observed-not-judged\n'
printf 'A.134 responsive A.133 continuation: %s\n' "$OUT"
