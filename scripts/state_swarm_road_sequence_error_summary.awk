# A.102: aggregate whichever sequence-error cohorts have been admitted.
...
END {
    ...
    for (p = 1; p <= policy_count; p++) {
        ...
        for (c = 1; c <= 2; c++) {
            ...
            expected_primary = cohort == "discovery" ? 0 : expected / 2
            expected_holdout = cohort == "discovery" ? expected : expected / 2
            ...
            print candidate, decay[candidate], strength[candidate], rank[candidate], \
                ...
                sprintf("%.9f", split_lives[key SUBSEP "primary"] ? split_raw_gain[key SUBSEP "primary"] / split_lives[key SUBSEP "primary"] : 0), \
                sprintf("%.9f", split_lives[key SUBSEP "holdout"] ? split_raw_gain[key SUBSEP "holdout"] / split_lives[key SUBSEP "holdout"] : 0), \
                sprintf("%.9f", split_lives[key SUBSEP "primary"] ? split_snapshot_gain[key SUBSEP "primary"] / split_lives[key SUBSEP "primary"] : 0), \
                sprintf("%.9f", split_lives[key SUBSEP "holdout"] ? split_snapshot_gain[key SUBSEP "holdout"] / split_lives[key SUBSEP "holdout"] : 0)
        }
    }
}
