#!/usr/bin/env bash
# A.107: test whether a realized consequence leaves a delayed state receipt.
set -Eeuo pipefail

trap 'rc=$?; printf "road delayed-consequence-receipt runner failed: line=%s rc=%s command=%s\n" "$LINENO" "$rc" "$BASH_COMMAND" >&2; exit "$rc"' ERR

ROOT="$(cd "$(dirname "$0")/.." && pwd)"
CC="${CC:-cc}"
RESERVOIR="${LEO_STATE_DELAYED_RECEIPT_RESERVOIR_SOURCE:-/private/tmp/leo-state-swarm-delayed-receipt-reservoir-a107-r1-20260813}"
A101="${LEO_STATE_DELAYED_RECEIPT_A101_SOURCE:-/private/tmp/leo-state-swarm-road-error-memory-a101-r1-20260810}"
A103="${LEO_STATE_DELAYED_RECEIPT_A103_SOURCE:-/private/tmp/leo-state-swarm-road-episode-memory-a103-r2-20260813}"
A104="${LEO_STATE_DELAYED_RECEIPT_A104_SOURCE:-/private/tmp/leo-state-swarm-road-ordered-episode-a104-r1-20260813}"
A105="${LEO_STATE_DELAYED_RECEIPT_A105_SOURCE:-/private/tmp/leo-state-swarm-road-episode-consequence-a105-r1-20260813}"
A106="${LEO_STATE_DELAYED_RECEIPT_A106_SOURCE:-/private/tmp/leo-state-swarm-road-counterbalanced-consequence-a106-r2-20260813}"
JOBS="${LEO_STATE_DELAYED_RECEIPT_JOBS:-4}"
AGGREGATE_ONLY="${LEO_STATE_DELAYED_RECEIPT_AGGREGATE_ONLY:-0}"
STAMP="$(date +%Y%m%d-%H%M%S)"
OUT="${1:-${TMPDIR:-/tmp}/leo-state-swarm-road-delayed-consequence-receipt-$STAMP}"

CANDIDATES="$ROOT/scripts/state_swarm_delayed_receipt_reservoir_candidates.tsv"
...
RESERVOIR_WRAPPER="$ROOT/scripts/state_swarm_delayed_receipt_reservoir_matrix.sh"
ELIGIBILITY_REPORTER="$ROOT/scripts/state_swarm_delayed_receipt_eligibility.awk"
RECEIPT_REPORTER="$ROOT/scripts/state_swarm_road_delayed_consequence_receipt_report.awk"
RECEIPT_LIFE="$ROOT/scripts/state_swarm_road_delayed_consequence_receipt_life.awk"
RECEIPT_SUMMARY="$ROOT/scripts/state_swarm_road_delayed_consequence_receipt_summary.awk"
RECEIPT_SELECTOR="$ROOT/scripts/state_swarm_road_delayed_consequence_receipt_select.awk"
RECEIPT_VERDICT="$ROOT/scripts/state_swarm_road_delayed_consequence_receipt_verdict.awk"
...
EXPECTED_CANDIDATES_SHA="${LEO_STATE_DELAYED_RECEIPT_CANDIDATES_SHA:-8de0de4b3caee560fad00b29fdb13527c488fd21842f74f0277850c7b3f34651}"
EXPECTED_WARM_CASES_SHA="${LEO_STATE_DELAYED_RECEIPT_WARM_CASES_SHA:-1f6e57f2ab55660b8a4cf2cc3e0a2769fa96ffd00ad0d8623e699b318acb4c0b}"
EXPECTED_WRITER_CASES_SHA="${LEO_STATE_DELAYED_RECEIPT_WRITER_CASES_SHA:-cfa13bb521d91a6a9f3db7fc278927f9120cd699fe72ce1a149307c170ea45e2}"
EXPECTED_REPORTER_SHA="${LEO_STATE_DELAYED_RECEIPT_REPORTER_SHA:-4667e61986b9fd43c62098786ddc28c9d79ecf5dfd1f74ecd1128b587005af80}"
EXPECTED_PLANNER_SHA="${LEO_STATE_DELAYED_RECEIPT_PLANNER_SHA:-2ccdaaeb73abad66c5f359dc927cb12a813a66f9489bf9017ba32728fd16b2a5}"
EXPECTED_PROSPECTIVE_SHA="${LEO_STATE_DELAYED_RECEIPT_PROSPECTIVE_SHA:-648c4202fc2c86d495b9e97568d9b982940beac7058f196f2c96fb9b40a9a181}"
EXPECTED_RESERVOIR_WRAPPER_SHA="${LEO_STATE_DELAYED_RECEIPT_RESERVOIR_WRAPPER_SHA:-53cfb31a0369b91fb36008a7be6e8c08a931a1771b567d43690f2e2d1d116b00}"
EXPECTED_ELIGIBILITY_REPORTER_SHA="${LEO_STATE_DELAYED_RECEIPT_ELIGIBILITY_REPORTER_SHA:-737dc716b751ab475cbafc0640a3eab47c8e8ce620d5c6aba3be1dbe4d67065b}"
EXPECTED_RECEIPT_REPORTER_SHA="${LEO_STATE_DELAYED_RECEIPT_RECEIPT_REPORTER_SHA:-2fd19352fbbae410625f14be01ce9f96aa6b29694a82b101399a43c1ceb1be56}"
EXPECTED_RECEIPT_LIFE_SHA="${LEO_STATE_DELAYED_RECEIPT_RECEIPT_LIFE_SHA:-d0ba24eceb2ac5c391d875ddd5041332cc04ad897a2d2d70eaa85fa6c3b5b491}"
EXPECTED_RECEIPT_SUMMARY_SHA="${LEO_STATE_DELAYED_RECEIPT_RECEIPT_SUMMARY_SHA:-edd333ccffa24824bdf70ae8fa0cccf23f590bf617617b7f37a0aa4090383c94}"
EXPECTED_RECEIPT_SELECTOR_SHA="${LEO_STATE_DELAYED_RECEIPT_RECEIPT_SELECTOR_SHA:-8599aec390aafc566af99343bc9d649273b4d4411c969a905f2bb218f376db6e}"
EXPECTED_RECEIPT_VERDICT_SHA="${LEO_STATE_DELAYED_RECEIPT_RECEIPT_VERDICT_SHA:-4c2d1675869c7dcbce192e9f19c82552bd777ee81e0e367f9d19aff76f9ef585}"
EXPECTED_LEO_SOURCE_SHA="${LEO_STATE_DELAYED_RECEIPT_LEO_SOURCE_SHA:-755f5f0a1e803f36b29ead5517086137af3696baf1959ad34bbca940316b8e73}"
EXPECTED_LEO_CORPUS_SHA="${LEO_STATE_DELAYED_RECEIPT_LEO_CORPUS_SHA:-98b3052353fc30e8bd08d3c2368ca1aaea41f9fb45b5f7cc2c6a17b78a9b4c19}"
EXPECTED_AML_SOURCE_SHA="${LEO_STATE_DELAYED_RECEIPT_AML_SOURCE_SHA:-a5cdb03356c9a51cbc8cc664333381cb975bd8c38622f30d293bb93e62ecb54d}"
EXPECTED_AML_HEADER_SHA="${LEO_STATE_DELAYED_RECEIPT_AML_HEADER_SHA:-b333a334658bbd1516cd9be79ca78c9e0cfd8acbfb90372a3c24af32cc0cca4a}"
EXPECTED_DIALOGUE_REPORTER_SHA="${LEO_STATE_DELAYED_RECEIPT_DIALOGUE_REPORTER_SHA:-35e393658785740430eec603459dac5ec3bfc1dc8bbac37f531de16582fe4c5e}"
EXPECTED_FIXTURE_SOURCE_SHA="${LEO_STATE_DELAYED_RECEIPT_FIXTURE_SOURCE_SHA:-6876954c90540ed92aa91a8beb47b433cee231207796b7d84c42c9f2dd9afa47}"
EXPECTED_RESERVOIR_SCREEN_PLAN_SHA="${LEO_STATE_DELAYED_RECEIPT_RESERVOIR_SCREEN_PLAN_SHA:-886e9ba571a9db6f98a5dae0958d4e8bebfd75a01b4af0caeb5206ed4eb6c0fa}"
EXPECTED_RESERVOIR_WARM_RECEIPTS_SHA="${LEO_STATE_DELAYED_RECEIPT_RESERVOIR_WARM_RECEIPTS_SHA:-84e28f25bd3941b7b2bdf48f909f0535795ca4ded760663b8810ed48fb32bbd3}"
EXPECTED_RESERVOIR_ENROLLMENT_SHA="${LEO_STATE_DELAYED_RECEIPT_RESERVOIR_ENROLLMENT_SHA:-e2d0595a4a53838145e498d97a3ab1423870896295eced2c93e069349106c890}"
EXPECTED_RESERVOIR_WRITER_PLAN_SHA="${LEO_STATE_DELAYED_RECEIPT_RESERVOIR_WRITER_PLAN_SHA:-339b7f2507b4836aa9429f00291376fce7e5b724fe36510ea11c283aecdccd75}"
EXPECTED_RESERVOIR_WRITER_RECEIPTS_SHA="${LEO_STATE_DELAYED_RECEIPT_RESERVOIR_WRITER_RECEIPTS_SHA:-906a2e68fa9c384a8e3e192855af1c34f67d60c49eaaaee1b45968425cae17c8}"
EXPECTED_RESERVOIR_VERDICT_SHA="${LEO_STATE_DELAYED_RECEIPT_RESERVOIR_VERDICT_SHA:-ab4d9ca41383a48e93abaf4d242905b1f1c595590fca1754a0bbf9acb27cffea}"
...
EXPECTED_A101_POLICIES_SHA="${LEO_STATE_DELAYED_RECEIPT_A101_POLICIES_SHA:-89736f8e4681db85b8c516f8095c16786d2e13ed1edbd4d3203244baff5529ab}"
EXPECTED_A101_DISCOVERY_PLAN_SHA="${LEO_STATE_DELAYED_RECEIPT_A101_DISCOVERY_PLAN_SHA:-ebe356a8b3055c6467a163a38e3893c115bfd39ca13ab6cff56ac802f013b860}"
EXPECTED_A101_VALIDATION_PLAN_SHA="${LEO_STATE_DELAYED_RECEIPT_A101_VALIDATION_PLAN_SHA:-259be3140c58c7706f985283734cc0c57b99d2d46f6221c17ee0adad6c026a82}"
EXPECTED_A101_DISCOVERY_SUMMARY_SHA="${LEO_STATE_DELAYED_RECEIPT_A101_DISCOVERY_SUMMARY_SHA:-05daa176f90ece7e6eb94016a9bb8e1122f7df0f1c6b2528e8d68643bb8fbb7a}"
EXPECTED_A101_SELECTION_SHA="${LEO_STATE_DELAYED_RECEIPT_A101_SELECTION_SHA:-0933c24077634e0e993a4019c628eecb5331138ebd6a26bde4eed99741c3364b}"
EXPECTED_A101_VERDICT_SHA="${LEO_STATE_DELAYED_RECEIPT_A101_VERDICT_SHA:-24053340a58e648b569de83255970334ad4fbb87e2f3d694bb1187203c0ffb29}"
...
EXPECTED_A103_POLICIES_SHA="${LEO_STATE_DELAYED_RECEIPT_A103_POLICIES_SHA:-a6a2e5f56ea3b33f166d9ec7869a78dbeebecc7f0545855060bdc7311d254272}"
EXPECTED_A103_SOURCE_RECEIPT_SHA="${LEO_STATE_DELAYED_RECEIPT_A103_SOURCE_RECEIPT_SHA:-2ccf516531fd8d046cdbd8a9c79f3fda59c5ee49595181244f9f644602325cfd}"
EXPECTED_A103_DISCOVERY_PLAN_SHA="${LEO_STATE_DELAYED_RECEIPT_A103_DISCOVERY_PLAN_SHA:-4e96a373c7e099f159d7e8655a54c4b55d3668e76c7c97796bada65a17cbb8a0}"
EXPECTED_A103_VALIDATION_PLAN_SHA="${LEO_STATE_DELAYED_RECEIPT_A103_VALIDATION_PLAN_SHA:-4e311548121664b65f8988368ca0dcddf5908dc0fc84c35cab09d03249618d98}"
EXPECTED_A103_DISCOVERY_SUMMARY_SHA="${LEO_STATE_DELAYED_RECEIPT_A103_DISCOVERY_SUMMARY_SHA:-76d8424893e2a120da82e632b098cdd709b536617030590b3ff3f5a5940e1fed}"
EXPECTED_A103_SELECTION_SHA="${LEO_STATE_DELAYED_RECEIPT_A103_SELECTION_SHA:-51d3ab48d0f19dea16a7f6309eb2c86110cc9e47b0b9e320e7ba36b8fb356637}"
EXPECTED_A103_VERDICT_SHA="${LEO_STATE_DELAYED_RECEIPT_A103_VERDICT_SHA:-6e7e0b5dd7672006e6296831e5a0bc71c6e4e13351029c7732c3dc66c2b0b192}"
...
EXPECTED_A104_POLICIES_SHA="${LEO_STATE_DELAYED_RECEIPT_A104_POLICIES_SHA:-7e0f0587d53651d637674dc73ef1cd69bef396be6f35c55c31101eb8bc915b6d}"
EXPECTED_A104_SOURCE_RECEIPT_SHA="${LEO_STATE_DELAYED_RECEIPT_A104_SOURCE_RECEIPT_SHA:-0a5d8c07893296ddeb591baabf8c2c32721fe98c499580288a418e747c8973ee}"
EXPECTED_A104_DISCOVERY_PLAN_SHA="${LEO_STATE_DELAYED_RECEIPT_A104_DISCOVERY_PLAN_SHA:-b4ff5f967346fb796e19e86370fdb5f73639be3fe956d95699398e7f81e2e169}"
EXPECTED_A104_VALIDATION_PLAN_SHA="${LEO_STATE_DELAYED_RECEIPT_A104_VALIDATION_PLAN_SHA:-79802ab37ca79bfe669a8c1ef8e836cd5749c4aae151a511289ac1fef2182a46}"
EXPECTED_A104_DISCOVERY_SUMMARY_SHA="${LEO_STATE_DELAYED_RECEIPT_A104_DISCOVERY_SUMMARY_SHA:-920f385dfb98b78bb93d60245261ecde4db9003321981a6f96248a546151fe7c}"
EXPECTED_A104_SELECTION_SHA="${LEO_STATE_DELAYED_RECEIPT_A104_SELECTION_SHA:-13c87f6f4f16d776994cfcc7ff20288bb3472ee59d1f8e75f6e849b758dea8f6}"
EXPECTED_A104_VERDICT_SHA="${LEO_STATE_DELAYED_RECEIPT_A104_VERDICT_SHA:-60b876a9cc510ac7f4f732d0328b26aa76f79c95b46eef47e3268bea62c65520}"
...
EXPECTED_A105_POLICIES_SHA="${LEO_STATE_DELAYED_RECEIPT_A105_POLICIES_SHA:-ab3aa8c577b93466b02f657113c654cd4357d4ecf0f09d458f4ebb0cf172f7c4}"
EXPECTED_A105_SOURCE_RECEIPT_SHA="${LEO_STATE_DELAYED_RECEIPT_A105_SOURCE_RECEIPT_SHA:-ad55334596982dca3cba6b275fc3ead9ab3a4812ee45512b2c101e0610154110}"
EXPECTED_A105_DISCOVERY_PLAN_SHA="${LEO_STATE_DELAYED_RECEIPT_A105_DISCOVERY_PLAN_SHA:-4476c0b318e5fc418d6eb819b51e9acbc55a95862adbc4cf98c0cd81794a4e75}"
EXPECTED_A105_VALIDATION_PLAN_SHA="${LEO_STATE_DELAYED_RECEIPT_A105_VALIDATION_PLAN_SHA:-82fc866a958b81c8520933f09836815fb9283fb0d78e88fba0c9cbcc1288d754}"
EXPECTED_A105_DISCOVERY_SUMMARY_SHA="${LEO_STATE_DELAYED_RECEIPT_A105_DISCOVERY_SUMMARY_SHA:-00f9f1adb3512f6e066016ee371324b34b58646922ce50b5f0b85803f2daab6f}"
EXPECTED_A105_SELECTION_SHA="${LEO_STATE_DELAYED_RECEIPT_A105_SELECTION_SHA:-da33255ff64af6b03c69258ac04eab40bd4f928c32aeeca4d9559978a299efdd}"
EXPECTED_A105_VERDICT_SHA="${LEO_STATE_DELAYED_RECEIPT_A105_VERDICT_SHA:-689d2a884897eaf48041f0ec004fcc836474a86a4f0eea2da71ac56356968722}"
...
EXPECTED_A106_POLICIES_SHA="${LEO_STATE_DELAYED_RECEIPT_A106_POLICIES_SHA:-ab3aa8c577b93466b02f657113c654cd4357d4ecf0f09d458f4ebb0cf172f7c4}"
EXPECTED_A106_DESIGN_SHA="${LEO_STATE_DELAYED_RECEIPT_A106_DESIGN_SHA:-2364fc67b61764c5f4ab5232b6bc028116e84a634c169a0a157f69c2743b89ea}"
EXPECTED_A106_SOURCE_RECEIPT_SHA="${LEO_STATE_DELAYED_RECEIPT_A106_SOURCE_RECEIPT_SHA:-816b16d88adfc2fa8ea478ea3a047aaae3e675063dfdfb21204af9f37d34473d}"
EXPECTED_A106_DISCOVERY_PLAN_SHA="${LEO_STATE_DELAYED_RECEIPT_A106_DISCOVERY_PLAN_SHA:-b1b5e0b15d7fc7309f1993e986a8887ec5910bbcb18231b4794b3929d7f70221}"
EXPECTED_A106_VALIDATION_PLAN_SHA="${LEO_STATE_DELAYED_RECEIPT_A106_VALIDATION_PLAN_SHA:-75fb4a949636787b8e37cf1ead9cb49c906256b71d5bc80dce857f726a3f38a4}"
EXPECTED_A106_DISCOVERY_SUMMARY_SHA="${LEO_STATE_DELAYED_RECEIPT_A106_DISCOVERY_SUMMARY_SHA:-f75bbc50e8a01296d5098f98f415bc556bdeaf60bb3328a89e310e8f5b5bdce2}"
EXPECTED_A106_SELECTION_SHA="${LEO_STATE_DELAYED_RECEIPT_A106_SELECTION_SHA:-6ab08012cd7438e3e13651685a1a872fde61551909432452b903f4073612d32e}"
EXPECTED_A106_VERDICT_SHA="${LEO_STATE_DELAYED_RECEIPT_A106_VERDICT_SHA:-46ffc17fdc85413ef607467e3eb2c6a199e56f142ffb020f65253770b1059c79}"

case "$JOBS" in
    ''|*[!0-9]*|0) printf 'invalid road delayed-consequence-receipt jobs: %s\n' "$JOBS" >&2; exit 2 ;;
esac
...
seal "$CANDIDATES" "$EXPECTED_CANDIDATES_SHA" "A.107 reservoir candidates"
seal "$WARM_CASES" "$EXPECTED_WARM_CASES_SHA" "A.107 warm cases"
seal "$WRITER_CASES" "$EXPECTED_WRITER_CASES_SHA" "A.107 writer cases"
seal "$REPORTER" "$EXPECTED_REPORTER_SHA" "A.107 reservoir reporter"
seal "$PLANNER" "$EXPECTED_PLANNER_SHA" "A.107 Williams planner"
seal "$PROSPECTIVE" "$EXPECTED_PROSPECTIVE_SHA" "A.107 prospective runner"
seal "$RESERVOIR_WRAPPER" "$EXPECTED_RESERVOIR_WRAPPER_SHA" "A.107 reservoir wrapper"
seal "$ELIGIBILITY_REPORTER" "$EXPECTED_ELIGIBILITY_REPORTER_SHA" "A.107 eligibility reporter"
seal "$RECEIPT_REPORTER" "$EXPECTED_RECEIPT_REPORTER_SHA" "A.107 receipt reporter"
seal "$RECEIPT_LIFE" "$EXPECTED_RECEIPT_LIFE_SHA" "A.107 receipt life reducer"
seal "$RECEIPT_SUMMARY" "$EXPECTED_RECEIPT_SUMMARY_SHA" "A.107 receipt summary"
seal "$RECEIPT_SELECTOR" "$EXPECTED_RECEIPT_SELECTOR_SHA" "A.107 receipt selector"
seal "$RECEIPT_VERDICT" "$EXPECTED_RECEIPT_VERDICT_SHA" "A.107 receipt verdict"
seal "$LEO_SOURCE" "$EXPECTED_LEO_SOURCE_SHA" "A.107 Leo source"
seal "$LEO_CORPUS" "$EXPECTED_LEO_CORPUS_SHA" "A.107 Leo corpus"
seal "$AML_SOURCE" "$EXPECTED_AML_SOURCE_SHA" "A.107 AML source"
seal "$AML_HEADER" "$EXPECTED_AML_HEADER_SHA" "A.107 AML header"
seal "$DIALOGUE_REPORTER" "$EXPECTED_DIALOGUE_REPORTER_SHA" "A.107 dialogue reporter"
seal "$FIXTURE_SOURCE" "$EXPECTED_FIXTURE_SOURCE_SHA" "A.107 geometry fixture"
...
if [ ! -d "$RESERVOIR" ]; then
    [ "$AGGREGATE_ONLY" = 0 ] || {
        printf 'delayed receipt reservoir source missing during aggregate-only replay: %s\n' "$RESERVOIR" >&2
        exit 2
    }
...
for path in "$SCREEN_PLAN" "$WARM_RECEIPTS" "$ENROLLMENT" \
    ...
    [ -s "$path" ] || { printf 'incomplete delayed receipt reservoir source: %s\n' "$path" >&2; exit 2; }
done
seal "$SCREEN_PLAN" "$EXPECTED_RESERVOIR_SCREEN_PLAN_SHA" "A.107 reservoir screen plan"
seal "$WARM_RECEIPTS" "$EXPECTED_RESERVOIR_WARM_RECEIPTS_SHA" "A.107 reservoir warm receipts"
seal "$ENROLLMENT" "$EXPECTED_RESERVOIR_ENROLLMENT_SHA" "A.107 reservoir enrollment"
seal "$WRITER_PLAN" "$EXPECTED_RESERVOIR_WRITER_PLAN_SHA" "A.107 reservoir writer plan"
seal "$WRITER_RECEIPTS" "$EXPECTED_RESERVOIR_WRITER_RECEIPTS_SHA" "A.107 reservoir writer receipts"
seal "$RESERVOIR_VERDICT" "$EXPECTED_RESERVOIR_VERDICT_SHA" "A.107 reservoir verdict"
grep -q '^result=balanced-reservoir-anatomy-admissible$' "$RESERVOIR_VERDICT" || {
    printf 'delayed receipt reservoir failed its own admission\n' >&2
    exit 2
}
awk -F '\t' '
...
' "$WRITER_CASES" "$ENROLLMENT" "$WRITER_PLAN" || {
    printf 'delayed receipt reservoir does not balance the scored calendar\n' >&2
    exit 2
}
...
FIXTURE="$OUT/road-delayed-consequence-receipt-fixture"
...
write_policies() {
    ...
    printf 'receipt-light\t0.10\t1.00\t0.25\t0.25\t1\t1\t1\n'
    printf 'receipt-gentle\t0.25\t1.00\t0.25\t0.25\t1\t1\t2\n'
}
...
write_design() {
    ...
    printf 'minimum-scored-receipts\t1\n'
...
if [ "$AGGREGATE_ONLY" = 1 ]; then
    for path in "$DISCOVERY_PLAN" "$VALIDATION_PLAN" "$POLICIES" "$DESIGN" \
        ...
        [ -s "$path" ] || { printf 'incomplete delayed-consequence-receipt run: %s\n' "$path" >&2; exit 2; }
    done
    write_discovery_plan | cmp -s - "$DISCOVERY_PLAN" || { printf 'A.107 discovery plan diverged\n' >&2; exit 2; }
    write_validation_plan | cmp -s - "$VALIDATION_PLAN" || { printf 'A.107 validation plan diverged\n' >&2; exit 2; }
    write_policies | cmp -s - "$POLICIES" || { printf 'A.107 policy ledger diverged\n' >&2; exit 2; }
    write_design | cmp -s - "$DESIGN" || { printf 'A.107 design ledger diverged\n' >&2; exit 2; }
    write_source_receipt | cmp -s - "$SOURCE_RECEIPT" || { printf 'A.107 source receipt diverged\n' >&2; exit 2; }
    write_eligibility | cmp -s - "$ELIGIBILITY" || { printf 'A.107 eligibility ledger diverged\n' >&2; exit 2; }
else
...
if [ "${LEO_STATE_DELAYED_RECEIPT_PLAN_ONLY:-0}" = 1 ]; then
...
awk -v policy_expected=2 -v life_expected=32 -v writer_expected=64 \
    -v evaluation_session=5 -v score_min=1 \
    -f "$RECEIPT_REPORTER" \
    "$POLICIES" "$DISCOVERY_LOCKS" "$DISCOVERY_WITNESSES" > "$DISCOVERY_SCORES"
awk -v policy_expected=2 -v life_expected=32 -v min_receipts=1 \
    -f "$RECEIPT_LIFE" \
...
[ -n "$selected" ] || { printf 'A.107 selection is empty\n' >&2; exit 2; }

if [ "$selected" != none ]; then
    { sed -n '1p' "$POLICIES"; awk -F '\t' -v selected="$selected" '$1 == selected' "$POLICIES"; } \
        > "$SELECTED_POLICY"
    [ "$(wc -l < "$SELECTED_POLICY" | tr -d ' ')" -eq 2 ] || {
        printf 'selected delayed-consequence-receipt policy is not unique\n' >&2; exit 2;
    }
    if [ "$AGGREGATE_ONLY" != 1 ]; then
    ...
    awk -v policy_expected=1 -v life_expected=32 -v min_receipts=1 \
        -f "$RECEIPT_LIFE" \
