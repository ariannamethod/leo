# A.108: ask whether a boundary displacement survives the signed path that followed it.

function fail() {
    if (!fatal) print "road delayed-receipt-path report rejected " FILENAME ":" FNR > "/dev/stderr"
    fatal = 1
    exit 2
}
