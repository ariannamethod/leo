...
for file in "$CANDIDATES" "$WARM_CASES" "$WRITER_CASES"; do
    [ -s "$file" ] || {
...
awk -F '\t' '
    ...
    {
        rows++
        expected_life = rows <= 30 ? sprintf("p%02d", rows) : sprintf("h%02d", rows - 30)
        expected_split = rows <= 30 ? "primary" : "holdout"
        expected_seed = 61001 + (rows - 1) * 1031
    ...
    END {
        if (rows != 40 || cohort_count["primary"] != 30 ||
            cohort_count["holdout"] != 10) exit 1
    }
' "$CANDIDATES" || {
...
awk -F '\t' '
    ...
    END {
        if (rows != 1280 || candidates != 40) exit 1
        target["primary"] = 24
        target["holdout"] = 8
...
if ! awk -F '\t' '
    ...
    END {
        if (rows != 32 || split_count["primary"] != 24 ||
            split_count["holdout"] != 8) exit 1
    }
' "$ENROLLMENT"; then
    {
        printf 'state-swarm prospective displacement incidence A.88\n'
        awk -F '\t' 'NR > 1 { candidates[$2]++; if ($11 == "true") settled[$2]++; if ($12 == "true") enrolled[$2]++ }
            END { printf "screened_candidates=40 primary=30 holdout=10\n";
                  printf "settled_candidates=%d/40 primary=%d/30 holdout=%d/10\n", settled["primary"] + settled["holdout"], settled["primary"] + 0, settled["holdout"] + 0;
...
awk -F '\t' '
    ...
    END {
        if (rows != 2048 || length(lives) != 32) exit 1
        for (life in lives)
...
running=0
while IFS=$'\t' read -r life split base_seed candidate_order enrollment_rank; do
    run_writer_life "$life" "$split" "$base_seed" &
    pids+=("$!")
    running=$((running + 1))
    if [ "$running" -ge "$JOBS" ]; then
        for pid in "${pids[@]}"; do wait "$pid"; done
...
awk -F '\t' '
    ...
    END {
        if (rows != 2048 || enrolled_count != 32) exit 1
...
awk -f "$ROOT/scripts/state_swarm_prospective_incidence_report.awk" \
    "$SCREENING" "$LIFE_SUMMARY" > "$VERDICT"
