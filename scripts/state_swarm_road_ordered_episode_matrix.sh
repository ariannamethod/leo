#!/usr/bin/env bash
# A.104: test whether temporal direction matters beyond unordered adjacency.
set -Eeuo pipefail

trap 'rc=$?; printf "road ordered-episode runner failed: line=%s rc=%s command=%s\n" "$LINENO" "$rc" "$BASH_COMMAND" >&2; exit "$rc"' ERR

ROOT="$(cd "$(dirname "$0")/.." && pwd)"
CC="${CC:-cc}"
RENEWAL="${LEO_STATE_ORDERED_RENEWAL_SOURCE:-/private/tmp/leo-state-swarm-renewal-event-reservoir-a103-r1-20260813}"
A101="${LEO_STATE_ORDERED_A101_SOURCE:-/private/tmp/leo-state-swarm-road-error-memory-a101-r1-20260810}"
A103="${LEO_STATE_ORDERED_A103_SOURCE:-/private/tmp/leo-state-swarm-road-episode-memory-a103-r2-20260813}"
JOBS="${LEO_STATE_ORDERED_JOBS:-4}"
AGGREGATE_ONLY="${LEO_STATE_ORDERED_AGGREGATE_ONLY:-0}"
STAMP="$(date +%Y%m%d-%H%M%S)"
OUT="${1:-${TMPDIR:-/tmp}/leo-state-swarm-road-ordered-path-$STAMP}"
...
EXPECTED_CANDIDATES_SHA="${LEO_STATE_ORDERED_CANDIDATES_SHA:-b32d476bf6fc990cb310b4ad9afed698c111cd5970ae8f9569cfa06230c7abf0}"
EXPECTED_WARM_CASES_SHA="${LEO_STATE_ORDERED_WARM_CASES_SHA:-1f6e57f2ab55660b8a4cf2cc3e0a2769fa96ffd00ad0d8623e699b318acb4c0b}"
EXPECTED_WRITER_CASES_SHA="${LEO_STATE_ORDERED_WRITER_CASES_SHA:-cfa13bb521d91a6a9f3db7fc278927f9120cd699fe72ce1a149307c170ea45e2}"
EXPECTED_REPORTER_SHA="${LEO_STATE_ORDERED_REPORTER_SHA:-4667e61986b9fd43c62098786ddc28c9d79ecf5dfd1f74ecd1128b587005af80}"
EXPECTED_RENEWAL_SCREEN_PLAN_SHA="${LEO_STATE_ORDERED_RENEWAL_SCREEN_PLAN_SHA:-1669e5984bafc9bc2e4dc6b489e63cff420a7575e9e2a1e6f262213ab0bb32cf}"
EXPECTED_RENEWAL_WARM_RECEIPTS_SHA="${LEO_STATE_ORDERED_RENEWAL_WARM_RECEIPTS_SHA:-0029de1008305fb04195d1adc094de7ef8e073a4d2e5848422a5b989b009b734}"
EXPECTED_RENEWAL_ENROLLMENT_SHA="${LEO_STATE_ORDERED_RENEWAL_ENROLLMENT_SHA:-374433ae590723873afe70a40418e307d844d6c5704460b98880ac7e8018e983}"
EXPECTED_RENEWAL_WRITER_PLAN_SHA="${LEO_STATE_ORDERED_RENEWAL_WRITER_PLAN_SHA:-bbf17ff252a8f9499ed081282d2908063b61bcbf0637dfa4b8d1c112742bed0b}"
EXPECTED_RENEWAL_WRITER_RECEIPTS_SHA="${LEO_STATE_ORDERED_RENEWAL_WRITER_RECEIPTS_SHA:-365f9a365a3ecec4f7c0a91a941a8fae513af83878a9df5276138e09895826d9}"
EXPECTED_RENEWAL_VERDICT_SHA="${LEO_STATE_ORDERED_RENEWAL_VERDICT_SHA:-517cf9d605f63a5c320bfe194ef7fc3b53588da3ec445f7d99f303f2fb3b4ef2}"
...
EXPECTED_A101_POLICIES_SHA="${LEO_STATE_ORDERED_A101_POLICIES_SHA:-89736f8e4681db85b8c516f8095c16786d2e13ed1edbd4d3203244baff5529ab}"
EXPECTED_A101_DISCOVERY_PLAN_SHA="${LEO_STATE_ORDERED_A101_DISCOVERY_PLAN_SHA:-ebe356a8b3055c6467a163a38e3893c115bfd39ca13ab6cff56ac802f013b860}"
EXPECTED_A101_VALIDATION_PLAN_SHA="${LEO_STATE_ORDERED_A101_VALIDATION_PLAN_SHA:-259be3140c58c7706f985283734cc0c57b99d2d46f6221c17ee0adad6c026a82}"
EXPECTED_A101_DISCOVERY_SUMMARY_SHA="${LEO_STATE_ORDERED_A101_DISCOVERY_SUMMARY_SHA:-05daa176f90ece7e6eb94016a9bb8e1122f7df0f1c6b2528e8d68643bb8fbb7a}"
EXPECTED_A101_SELECTION_SHA="${LEO_STATE_ORDERED_A101_SELECTION_SHA:-0933c24077634e0e993a4019c628eecb5331138ebd6a26bde4eed99741c3364b}"
EXPECTED_A101_VERDICT_SHA="${LEO_STATE_ORDERED_A101_VERDICT_SHA:-24053340a58e648b569de83255970334ad4fbb87e2f3d694bb1187203c0ffb29}"
...
EXPECTED_A103_POLICIES_SHA="${LEO_STATE_ORDERED_A103_POLICIES_SHA:-a6a2e5f56ea3b33f166d9ec7869a78dbeebecc7f0545855060bdc7311d254272}"
EXPECTED_A103_SOURCE_RECEIPT_SHA="${LEO_STATE_ORDERED_A103_SOURCE_RECEIPT_SHA:-2ccf516531fd8d046cdbd8a9c79f3fda59c5ee49595181244f9f644602325cfd}"
EXPECTED_A103_DISCOVERY_PLAN_SHA="${LEO_STATE_ORDERED_A103_DISCOVERY_PLAN_SHA:-4e96a373c7e099f159d7e8655a54c4b55d3668e76c7c97796bada65a17cbb8a0}"
EXPECTED_A103_VALIDATION_PLAN_SHA="${LEO_STATE_ORDERED_A103_VALIDATION_PLAN_SHA:-4e311548121664b65f8988368ca0dcddf5908dc0fc84c35cab09d03249618d98}"
EXPECTED_A103_DISCOVERY_SUMMARY_SHA="${LEO_STATE_ORDERED_A103_DISCOVERY_SUMMARY_SHA:-76d8424893e2a120da82e632b098cdd709b536617030590b3ff3f5a5940e1fed}"
EXPECTED_A103_SELECTION_SHA="${LEO_STATE_ORDERED_A103_SELECTION_SHA:-51d3ab48d0f19dea16a7f6309eb2c86110cc9e47b0b9e320e7ba36b8fb356637}"
EXPECTED_A103_VERDICT_SHA="${LEO_STATE_ORDERED_A103_VERDICT_SHA:-6e7e0b5dd7672006e6296831e5a0bc71c6e4e13351029c7732c3dc66c2b0b192}"

case "$JOBS" in
    ''|*[!0-9]*|0) printf 'invalid road ordered-episode jobs: %s\n' "$JOBS" >&2; exit 2 ;;
esac
...
FIXTURE="$OUT/road-ordered-path-fixture"

write_discovery_plan() {
    awk -F '\t' -v OFS='\t' '
        NR == 1 { print "cohort", $0; next }
        $5 >= 12 && $5 <= 17 { print "discovery", $0; rows++; split_count[$2]++ }
        END {
...
write_validation_plan() {
    awk -F '\t' -v OFS='\t' '
        NR == 1 { print "cohort", $0; next }
        $5 >= 18 && $5 <= 22 { print "validation", $0; rows++; split_count[$2]++ }
        END {
...
write_policies() {
    printf 'candidate\tpath_decay\tpath_strength\tsnapshot_decay\tsnapshot_strength\tprior_alpha\tvariance_ridge\trank\n'
    printf 'path-last-light\t0.00\t0.10\t1.00\t0.25\t1\t1\t1\n'
    printf 'path-last-gentle\t0.00\t0.25\t1.00\t0.25\t1\t1\t2\n'
    printf 'path-short-light\t0.50\t0.10\t1.00\t0.25\t1\t1\t3\n'
    printf 'path-short-gentle\t0.50\t0.25\t1.00\t0.25\t1\t1\t4\n'
    printf 'path-long-light\t0.85\t0.10\t1.00\t0.25\t1\t1\t5\n'
    printf 'path-long-gentle\t0.85\t0.25\t1.00\t0.25\t1\t1\t6\n'
}
...
if [ "$AGGREGATE_ONLY" = 1 ]; then
    for path in "$DISCOVERY_PLAN" "$VALIDATION_PLAN" "$POLICIES" \
        ...
        [ -s "$path" ] || { printf 'incomplete ordered-episode run: %s\n' "$path" >&2; exit 2; }
    done
    write_discovery_plan | cmp -s - "$DISCOVERY_PLAN" || { printf 'A.104 discovery plan diverged\n' >&2; exit 2; }
    write_validation_plan | cmp -s - "$VALIDATION_PLAN" || { printf 'A.104 validation plan diverged\n' >&2; exit 2; }
    write_policies | cmp -s - "$POLICIES" || { printf 'A.104 policy ledger diverged\n' >&2; exit 2; }
    write_source_receipt | cmp -s - "$SOURCE_RECEIPT" || { printf 'A.104 source receipt diverged\n' >&2; exit 2; }
else
...
if [ "${LEO_STATE_ORDERED_PLAN_ONLY:-0}" = 1 ]; then
...
awk -f "$ROOT/scripts/state_swarm_road_ordered_episode_verdict.awk" \
