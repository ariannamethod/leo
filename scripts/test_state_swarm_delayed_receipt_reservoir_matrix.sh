...
TMP="$(mktemp -d "${TMPDIR:-/tmp}/leo-delayed-receipt-reservoir-plan.XXXXXX")"
...
awk -F '\t' '
    ...
    }
    {
        ...
        expected_seed = 357923 + (rows - 1) * 1033
        if ($1 != expected_life || $2 != expected_split || $3 != expected_seed || seed[$3]++) exit 1
    }
    END { if (rows != 80) exit 1 }
' "$CANDIDATES"

previous_last="$(tail -n 1 "$ROOT/scripts/state_swarm_counterbalanced_episode_reservoir_candidates.tsv" | awk -F '\t' '{ print $3 }')"
new_first="$(sed -n '2p' "$CANDIDATES" | awk -F '\t' '{ print $3 }')"
[ "$new_first" -eq $((previous_last + 1033)) ]
...
awk -F '\t' '
    ...
    END {
        ...
        for (c = 1; c <= 2; c++) {
            ...
            for (score_block = 1; score_block <= 1; score_block++)
                for (chronological = 5; chronological <= 8; chronological++)
                    for (source_session = 1; source_session <= 8; source_session++)
                        if (score_position[cohort SUBSEP score_block SUBSEP chronological SUBSEP source_session] != 2)
                            exit 1
            for (score_block = 1; score_block <= 1; score_block++)
                for (source_session = 1; source_session <= 8; source_session++)
                    for (destination = 1; destination <= 8; destination++) {
                        observed = score_carry[cohort SUBSEP score_block SUBSEP source_session SUBSEP destination] + 0
...
awk -F '\t' 'BEGIN { OFS=FS } $1 == "h16" { $5 = 15 } { print }' \
    "$TMP/enrollment.tsv" > "$TMP/forged-enrollment.tsv"
if awk -v primary_expected=16 -v holdout_expected=16 \
    -f "$ROOT/scripts/state_swarm_williams8_writer_plan.awk" \
    "$CASES" "$TMP/forged-enrollment.tsv" >/dev/null 2>&1; then
    printf 'Williams writer planner accepted duplicate enrollment rank\n' >&2
    exit 1
fi

LEO_STATE_PROSPECTIVE_PLAN_ONLY=1 \
    "$ROOT/scripts/state_swarm_delayed_receipt_reservoir_matrix.sh" "$TMP/run" \
    > "$TMP/plan.stdout"
...
printf 'state-swarm delayed receipt reservoir plan: ok\n'
