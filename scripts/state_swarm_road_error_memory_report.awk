# A.101: learn a past-only correction to the raw road's completed errors.

function fail() {
    if (!fatal) print "road error-memory report rejected " FILENAME ":" FNR > "/dev/stderr"
    fatal = 1
    exit 2
}
...
FILENAME == ARGV[1] {
    ...
    if (NF != 6 || $1 !~ /^err-[a-z0-9-]+$/ || policy_seen[$1]++ ||
...

{
    ...
    if ($8 == "replaced") {
        ...
        for (p = 1; p <= policy_count; p++) {
            ...
            for (i = 1; i <= 8; i++) {
                ...
                delete error_mean[candidate SUBSEP life SUBSEP old_pre_id[i]]
    ...
    for (p = 1; p <= policy_count; p++) {
        candidate = policies[p]
        for (j = 1; j <= 8; j++) {
            destination_key = candidate SUBSEP life SUBSEP pre_id[j]
            bias_prediction[j] = raw_prediction[j] + strength[candidate] * error_mean[destination_key]
            candidate_prediction[j] = bias_prediction[j]
            for (i = 1; i <= 8; i++) {
                source_key = candidate SUBSEP life SUBSEP pre_id[i]
                centered_source = source[i] - source_mean[source_key]
                candidate_prediction[j] += strength[candidate] * centered_source * covariance[source_key SUBSEP pre_id[j]] / (ridge[candidate] + source_variance[source_key])
            }
        }
        normalize(bias_prediction)
        normalize(candidate_prediction)
        bias_ce = ce(target, bias_prediction)
        candidate_ce = ce(target, candidate_prediction)
        bias_brier = brier(target, bias_prediction)
        ...
        if ($4 >= evaluation_start) {
            if (!header++)
                print "candidate", "decay", "strength", "rank", "cohort", \
                    ...
                    "candidate_ce", "bias_ce", "raw_brier", \
                    "candidate_brier", "bias_brier", "raw_ce_gain", \
                    "raw_brier_gain", "bias_ce_gain", "bias_brier_gain", \
                    "bias_raw_ce_gain", "bias_raw_brier_gain"
            print candidate, decay[candidate], strength[candidate], rank[candidate], \
                $1, life, $3, $4, $7, sprintf("%.9f", raw_ce), \
                sprintf("%.9f", candidate_ce), sprintf("%.9f", bias_ce), \
                sprintf("%.9f", raw_brier), sprintf("%.9f", candidate_brier), \
                sprintf("%.9f", bias_brier), \
                sprintf("%.9f", raw_ce - candidate_ce), \
                sprintf("%.9f", raw_brier - candidate_brier), \
                sprintf("%.9f", bias_ce - candidate_ce), \
                sprintf("%.9f", bias_brier - candidate_brier), \
                sprintf("%.9f", raw_ce - bias_ce), \
                sprintf("%.9f", raw_brier - bias_brier)
        ...
        for (j = 1; j <= 8; j++) {
            ...
            error_delta[j] = observed_error[j] - error_mean[destination_key]
        }
        for (i = 1; i <= 8; i++) {
            source_key = candidate SUBSEP life SUBSEP pre_id[i]
            source_variance[source_key] = decay[candidate] * source_variance[source_key] + recenter * source_delta[i] * source_delta[i]
            for (j = 1; j <= 8; j++)
                covariance[source_key SUBSEP pre_id[j]] = decay[candidate] * covariance[source_key SUBSEP pre_id[j]] + recenter * source_delta[i] * error_delta[j]
        }
        for (i = 1; i <= 8; i++) {
            source_key = candidate SUBSEP life SUBSEP pre_id[i]
        ...
        for (j = 1; j <= 8; j++) {
            destination_key = candidate SUBSEP life SUBSEP pre_id[j]
            error_mean[destination_key] += error_delta[j] / new_weight
        }
