#!/usr/bin/env bash
# A.136: let a visible-only interlocutor answer Leo's bare Difficult return.
set -Eeuo pipefail

trap 'rc=$?; printf "responsive difficult return failed: line=%s rc=%s command=%s\n" "$LINENO" "$rc" "$BASH_COMMAND" >&2; exit "$rc"' ERR

ROOT="$(cd "$(dirname "$0")/.." && pwd)"
STAMP="$(date +%Y%m%d-%H%M%S)"
OUT="${1:-${TMPDIR:-/tmp}/leo-responsive-difficult-return-a136-$STAMP}"
...
if [ ! -f "$API/dialogue.jsonl" ]; then
    LEO_NATURAL_REPLAY_FILE="$PREFIX" \
        ...
        LEO_NATURAL_QUESTION=responsive-difficult-return \
...
cmp -s "$EXPECTED_PREFIX" "$OUT/prefix.actual.jsonl" || {
    printf 'A.135 twenty-five-turn prefix did not reproduce exactly\n' >&2
...
cat "$MATRIX"
printf 'result\tresponsive-difficult-return-observed-not-judged\n'
printf 'A.136 responsive difficult return: %s\n' "$OUT"
