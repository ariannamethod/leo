# A.105: ask whether a bounded episode's ending predicts its later consequence.

function fail() {
    if (!fatal) print "road episode-consequence report rejected " FILENAME ":" FNR > "/dev/stderr"
    fatal = 1
    exit 2
}
...
function start_epoch(life,    candidate, texture, i, j, k, key, current_epoch) {
    ...
    for (candidate in policy_seen) {
        ...
        for (k = 1; k <= 24; k++) {
            unordered_mean[key SUBSEP k] = k <= 16 ? 0.125 : 0
            unordered_variance[key SUBSEP k] = 0
            for (j = 1; j <= 8; j++)
                unordered_covariance[key SUBSEP k SUBSEP j] = 0
        }
        for (k = 1; k <= 8; k++) {
            ending_mean[key SUBSEP k] = 0
            ending_variance[key SUBSEP k] = 0
            for (j = 1; j <= 8; j++)
                ending_covariance[key SUBSEP k SUBSEP j] = 0
        }
        for (j = 1; j <= 8; j++) {
            unordered_error_mean[key SUBSEP j] = 0
            ending_error_mean[key SUBSEP j] = 0
        }
    }
}
...
FILENAME == ARGV[1] {
    if (FNR == 1) {
        if (NF != 8 || $1 != "candidate" || $2 != "consequence_strength" ||
            $3 != "snapshot_decay" || $5 != "texture_strength" ||
            $6 != "prior_alpha" || $8 != "rank") fail()
        next
    }
    if (NF != 8 || $1 !~ /^consequence-[a-z0-9-]+$/ || policy_seen[$1]++ ||
...

{
    ...
    episode_ready = $6 == 1 && $5 > 1 && \
        session_rows[previous_key] == 8 && !session_invalid[previous_key]
    ...
    if (episode_ready) {
        for (i = 1; i <= 8; i++) {
            episode_mean[i] = 0
            for (o = 1; o <= 8; o++)
                episode_mean[i] += session_source[previous_key SUBSEP o SUBSEP i] / 8
            episode_start[i] = session_source[previous_key SUBSEP 1 SUBSEP i]
            episode_end[i] = session_source[previous_key SUBSEP 8 SUBSEP i]
            unordered_feature[i] = episode_mean[i]
            unordered_feature[8 + i] = 0.5 * (episode_start[i] + episode_end[i])
            unordered_feature[16 + i] = abs(episode_end[i] - episode_start[i])
            ending_feature[i] = episode_end[i] - episode_start[i]
        }
        ...
        for (p = 1; p <= policy_count; p++) {
            ...
            for (j = 1; j <= 8; j++) {
                unordered_prediction[j] = texture_prediction[j]
                for (k = 1; k <= 24; k++) {
                    feature_key = reader_key SUBSEP k
                    unordered_prediction[j] += strength[candidate] * \
                        (unordered_feature[k] - unordered_mean[feature_key]) * \
                        unordered_covariance[feature_key SUBSEP j] / \
                        (ridge + unordered_variance[feature_key])
                }
            }
            normalize(unordered_prediction)
            for (j = 1; j <= 8; j++) {
                consequence_prediction[j] = unordered_prediction[j]
                for (k = 1; k <= 8; k++) {
                    feature_key = reader_key SUBSEP k
                    consequence_prediction[j] += strength[candidate] * \
                        (ending_feature[k] - ending_mean[feature_key]) * \
                        ending_covariance[feature_key SUBSEP j] / \
                        (ridge + ending_variance[feature_key])
                }
            }
            normalize(consequence_prediction)
            unordered_ce = ce(target, unordered_prediction)
            unordered_brier = brier(target, unordered_prediction)
            consequence_ce = ce(target, consequence_prediction)
            consequence_brier = brier(target, consequence_prediction)

            if ($5 >= evaluation_session) {
                if (!header++)
                    print "candidate", "consequence_strength", "rank", "cohort", \
                        "life", "split", "turn", "session", "texture", "raw_ce", \
                        "consequence_ce", "unordered_ce", "texture_ce", "snapshot_ce", \
                        "raw_brier", "consequence_brier", "unordered_brier", \
                        "texture_brier", "snapshot_brier", "raw_ce_gain", \
                        "raw_brier_gain", "snapshot_ce_gain", "snapshot_brier_gain", \
                        "texture_ce_gain", "texture_brier_gain", "unordered_ce_gain", \
                ...
                print candidate, strength[candidate], rank[candidate], $1, life, $3, \
                    ...
                    sprintf("%.9f", consequence_ce), sprintf("%.9f", unordered_ce), \
                    sprintf("%.9f", texture_ce), sprintf("%.9f", snapshot_ce), \
                    sprintf("%.9f", raw_brier), sprintf("%.9f", consequence_brier), \
                    sprintf("%.9f", unordered_brier), sprintf("%.9f", texture_brier), \
                    sprintf("%.9f", snapshot_brier), \
                    sprintf("%.9f", raw_ce - consequence_ce), \
                    sprintf("%.9f", raw_brier - consequence_brier), \
                    sprintf("%.9f", snapshot_ce - consequence_ce), \
                    sprintf("%.9f", snapshot_brier - consequence_brier), \
                    sprintf("%.9f", texture_ce - consequence_ce), \
                    sprintf("%.9f", texture_brier - consequence_brier), \
                    sprintf("%.9f", unordered_ce - consequence_ce), \
                    sprintf("%.9f", unordered_brier - consequence_brier), \
            ...
            for (k = 1; k <= 24; k++) {
                feature_key = reader_key SUBSEP k
                unordered_delta[k] = unordered_feature[k] - unordered_mean[feature_key]
            }
            for (k = 1; k <= 8; k++) {
                feature_key = reader_key SUBSEP k
                ending_delta[k] = ending_feature[k] - ending_mean[feature_key]
            }
            for (j = 1; j <= 8; j++) {
                unordered_error_delta[j] = target[j] - texture_prediction[j] - \
                    unordered_error_mean[reader_key SUBSEP j]
                ending_error_delta[j] = target[j] - unordered_prediction[j] - \
                    ending_error_mean[reader_key SUBSEP j]
            }
            for (k = 1; k <= 24; k++) {
                feature_key = reader_key SUBSEP k
                unordered_variance[feature_key] += recenter * unordered_delta[k] * unordered_delta[k]
                for (j = 1; j <= 8; j++)
                    unordered_covariance[feature_key SUBSEP j] += \
            ...
            for (k = 1; k <= 8; k++) {
                feature_key = reader_key SUBSEP k
                ending_variance[feature_key] += recenter * ending_delta[k] * ending_delta[k]
                for (j = 1; j <= 8; j++)
                    ending_covariance[feature_key SUBSEP j] += \
                        recenter * ending_delta[k] * ending_error_delta[j]
            }
            for (k = 1; k <= 24; k++)
                unordered_mean[reader_key SUBSEP k] += unordered_delta[k] / new_weight
            for (k = 1; k <= 8; k++)
                ending_mean[reader_key SUBSEP k] += ending_delta[k] / new_weight
            for (j = 1; j <= 8; j++) {
                unordered_error_mean[reader_key SUBSEP j] += unordered_error_delta[j] / new_weight
                ending_error_mean[reader_key SUBSEP j] += ending_error_delta[j] / new_weight
            }
