#!/usr/bin/env bash
# A.107 source: untouched Williams-balanced lives for delayed receipts.
set -Eeuo pipefail

ROOT="$(cd "$(dirname "$0")/.." && pwd)"
STAMP="$(date +%Y%m%d-%H%M%S)"
OUT="${1:-${TMPDIR:-/tmp}/leo-state-swarm-delayed-receipt-reservoir-$STAMP}"

export LEO_STATE_PROSPECTIVE_CANDIDATES="$ROOT/scripts/state_swarm_delayed_receipt_reservoir_candidates.tsv"
...
export LEO_STATE_PROSPECTIVE_SEED_START=357923
...
export LEO_STATE_PROSPECTIVE_PROFILE_NAME=A.107-delayed-receipt
