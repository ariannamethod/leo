# A.98: make each replayed life one statistical vote.
...
NR == 1 {
    if (NF != 19 || $1 != "candidate" || $5 != "cohort" ||
        $16 != "raw_ce_gain" || $19 != "destination_brier_gain") fail()
    next
}

{
    if (NF != 19 || $1 !~ /^[a-z][a-z0-9-]+$/ ||
        !number($2) || !number($3) || $4 !~ /^[0-9]+$/ ||
...
END {
    ...
    print "candidate", "decay", "strength", "rank", "cohort", "life", \
        "split", "turns", "turn_wins", "mean_raw_ce_gain", \
        "mean_raw_brier_gain", "mean_destination_ce_gain", \
    ...
    for (p = 1; p <= policy_count; p++) {
        ...
        for (l = 1; l <= life_count; l++) {
            ...
            print candidate, decay[candidate], strength[candidate], rank[candidate], \
                cohort[life], life, split_name[life], count[key], turn_wins[key] + 0, \
