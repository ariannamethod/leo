...
from typing import Dict, List, Optional, Tuple, NamedTuple, Set, Callable
...
class NeoLeo:
    ...
    def _overthinking_generate(
        ...
        Maps overthinking parameters to NeoLeo's generation machinery.
        Never prints to stdout - this is pure internal reflection.
        """
        try:
            # Use generate_reply with overthinking-specific settings
            output = generate_reply(
                self.bigrams,
                self.vocab,
                self.centers,
            ...
            return output if isinstance(output, str) else seed
        except Exception:
            # Silent fallback: overthinking must never break NeoLeo
            return seed
    ...
    def _overthinking_observe(self, text: str, source: str) -> None:
        ...
        Feeds overthinking rings back into NeoLeo's field without
